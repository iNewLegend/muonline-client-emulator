...
void DGGuildListRequest()
{
	...
	pMsg.h.set(0x40, sizeof(SDHP_GUILDLISTREQUEST));
...
void DGGuildServerGroupChattingRecv( EXSDHP_SERVERGROUP_GUILD_CHATTING_RECV* lpMsg)
{
	...
	pMsg.h.set(0x00, sizeof(pMsg));	// #error MAYBE the header is wrong
...
void FriendMemoReadReq(PMSG_FRIEND_READ_MEMO_REQ * lpMsg, int aIndex)
{
	...
	// LEO
		if(g_Manger.ReadMsgReq(aIndex, lpMsg->MemoIndex ))
		{
			LogAddTD("[CManager] using mail :: %d",lpMsg->MemoIndex);
			return;
		}
	// LEO
	pMsg.Number = aIndex;
...
void FriendMemoRead(FHP_FRIEND_MEMO_RECV * lpMsg)
{
	...
	// LEO
		eMsg->PMAll("MemoReadReq :: %d",lpMsg->MemoIndex);
	// LEO
