...
#ifdef _DEBUG
#define DATA_SIZE_CHECK(_struct_)	if (sizeof(_struct_)!=aLen) {LogAddC(3,"Error-L5 : Different sizes: struct=%d recv=%d",sizeof(_struct_),aLen);}
#endif
void ProtocolCore(BYTE protoNum, unsigned char * aRecv, int aLen, int aIndex, BOOL Encrypt, int serial)
{
	...
	{
	case 0xF1:
		switch ( ((PMSG_DEFAULT2 *)aRecv)->subcode )
		{
		case 0x01:
			if ( DataEncryptCheck(aIndex, protoNum, Encrypt) != FALSE )
			{
				DATA_SIZE_CHECK(PMSG_IDPASS);
		...
		case 0x02:
			if ( DataEncryptCheck(aIndex, protoNum, Encrypt) != FALSE )
			{
				DATA_SIZE_CHECK(PMSG_CLIENTCLOSE);
		...
		case 0x03:
			if ( DataEncryptCheck(aIndex, protoNum, Encrypt) != FALSE )
			{
				DATA_SIZE_CHECK(PMSG_CLIENTMSG);
	...
	case 0xF3:
		...
		case 0x12:
			...
			break;
		case 0x30:
			DATA_SIZE_CHECK(PMSG_SKILLKEY);
			GCSkillKeyRecv((PMSG_SKILLKEY *)aRecv, aIndex);
			break;
		}
		break;
	case 0xFF:
		DATA_SIZE_CHECK(PMSG_MAC_ADDR);
		PMacProc((PMSG_MAC_ADDR *)aRecv, aIndex);
		break;
	case 0x00:
		DATA_SIZE_CHECK(PMSG_CHATDATA);
		PChatProc((PMSG_CHATDATA *)aRecv, aIndex);
		break;
	case 0x01:
		DATA_SIZE_CHECK(PMSG_CHATDATA_NUMBER);
		CGChatRecv((PMSG_CHATDATA_NUMBER *)aRecv, aIndex);
		break;
	case 0x02:
		DATA_SIZE_CHECK(PMSG_CHATDATA_WHISPER);
		CGChatWhisperRecv((PMSG_CHATDATA_WHISPER *)aRecv, aIndex);
		break;
	case GMSG_CHECK_MAINEXE:
		DATA_SIZE_CHECK(PMSG_CHECK_MAINEXE_RESULT);
		CGCheckMainRecv((PMSG_CHECK_MAINEXE_RESULT *)aRecv, aIndex);
		break;
	case GMSG_CLIENT_TIME:
		DATA_SIZE_CHECK(PMSG_CLIENTTIME);
		CGLiveClient((PMSG_CLIENTTIME *)aRecv, aIndex);
		break;
	case GMSG_MOVE:
		DATA_SIZE_CHECK(PMSG_MOVE);
		PMoveProc((PMSG_MOVE *)aRecv, aIndex);
		break;
	case GMSG_POSISTION_SET:
		DATA_SIZE_CHECK(PMSG_POSISTION_SET);
		RecvPositionSetProc((PMSG_POSISTION_SET *)aRecv, aIndex);
		break;
	case GMSG_ATTACK:
		DATA_SIZE_CHECK(PMSG_ATTACK);
		CGAttack((PMSG_ATTACK *)aRecv, aIndex);
		break;
	case GMSG_ACTION:
		DATA_SIZE_CHECK(PMSG_ACTION);
		CGActionRecv((PMSG_ACTION *)aRecv, aIndex);
		break;
	case GMSG_MAGIC_ATTACK:
		if ( DataEncryptCheck(aIndex, protoNum, Encrypt) != FALSE )
		{
			DATA_SIZE_CHECK(PMSG_MAGICATTACK);
			CGMagicAttack((PMSG_MAGICATTACK *)aRecv, aIndex);
		}
		break;
	case GMSG_MAGIC_CANCEL:
		DATA_SIZE_CHECK(PMSG_MAGICCANCEL);
		CGMagicCancel((PMSG_MAGICCANCEL *)aRecv, aIndex);
		break;
	case GMSG_TELEPORT:
		if ( DataEncryptCheck(aIndex, protoNum, Encrypt) != FALSE )
		{
			DATA_SIZE_CHECK(PMSG_TELEPORT);
	...
	case 0xB0:
		if ( DataEncryptCheck(aIndex, protoNum, Encrypt) != FALSE )
		{
			DATA_SIZE_CHECK(PMSG_TARGET_TELEPORT);
	...
	case GMSG_BEATTACK:
		...
		break;
	case GMSG_DURATION_MAGIC:
		if ( DataEncryptCheck(aIndex, protoNum, Encrypt) != FALSE )
		{
			DATA_SIZE_CHECK(PMSG_DURATION_MAGIC_RECV);
	...
	case 0x32:
		if ( DataEncryptCheck(aIndex, protoNum, Encrypt) != FALSE )
		{
			DATA_SIZE_CHECK(PMSG_BUYREQUEST);
	...
	case 0x3F:
		...
		{
		case 0x01:
			DATA_SIZE_CHECK(PMSG_REQ_PSHOP_SETITEMPRICE);
			CGPShopReqSetItemPrice((PMSG_REQ_PSHOP_SETITEMPRICE *)aRecv, aIndex);
			break;
		case 0x02:
			DATA_SIZE_CHECK(PMSG_REQ_PSHOP_OPEN);
		...
		case 0x03:
			...
			break;
		case 0x05:
			DATA_SIZE_CHECK(PMSG_REQ_BUYLIST_FROM_PSHOP);
			CGPShopReqBuyList((PMSG_REQ_BUYLIST_FROM_PSHOP *)aRecv, aIndex);
			break;
		case 0x06:
			DATA_SIZE_CHECK(PMSG_REQ_BUYITEM_FROM_PSHOP);
	...
	case 0x40:
		if ( DataEncryptCheck(aIndex, protoNum, Encrypt) != FALSE )
		{
			DATA_SIZE_CHECK(PMSG_PARTYREQUEST);
	...
	case 0x41:
		if ( DataEncryptCheck(aIndex, protoNum, Encrypt) != FALSE )
		{
			DATA_SIZE_CHECK(PMSG_PARTYREQUESTRESULT);
	...
	case 0x43:
		...
		{
			DATA_SIZE_CHECK(PMSG_PARTYDELUSER);
			CGPartyDelUser((PMSG_PARTYDELUSER *)aRecv, aIndex);
		}
		break;
	case 0x50:
		DATA_SIZE_CHECK(PMSG_GUILDJOINQ);
		CGGuildRequestRecv((PMSG_GUILDJOINQ *)aRecv, aIndex);
		break;
	case 0x51:
		DATA_SIZE_CHECK(PMSG_GUILDQRESULT);
	...
	case 0x52:
		...
		break;
	case 0x53:
		DATA_SIZE_CHECK(PMSG_GUILDDELUSER);
		CGGuildDelUser((PMSG_GUILDDELUSER *)aRecv, aIndex);
		break;
	case 0x54:
		DATA_SIZE_CHECK(PMSG_GUILDMASTERANSWER);
		CGGuildMasterAnswerRecv((PMSG_GUILDMASTERANSWER *)aRecv, aIndex);
		break;
	case 0x55:
		DATA_SIZE_CHECK(PMSG_GUILDINFOSAVE);
	...
	case 0x57:
		...
		break;
	case 0x61:
		DATA_SIZE_CHECK(PMSG_GUILDWARSEND_RESULT);
		GCGuildWarRequestSendRecv((PMSG_GUILDWARSEND_RESULT *)aRecv, aIndex);
		break;
	case 0x66:
		DATA_SIZE_CHECK(PMSG_REQ_GUILDVIEWPORT);
	...
	case 0xB1:
		...
		case 0x01:
			if ( DataEncryptCheck(aIndex, protoNum, Encrypt) != FALSE )
			{
				DATA_SIZE_CHECK(PMSG_REQ_MAPSERVERAUTH);
	...
	case 0xB7:
		...
		{
		case 0x01:
			DATA_SIZE_CHECK(PMSG_REQ_USEWEAPON);
			CGReqWeaponUse((PMSG_REQ_USEWEAPON *)aRecv, aIndex);
			break;
		case 0x04:
			DATA_SIZE_CHECK(PMSG_REQ_WEAPON_DAMAGE_VALUE);
	...
	case 0xB9:
			...
			{
			case 0x02:
				DATA_SIZE_CHECK(PMSG_REQ_GUILDMARK_OF_CASTLEOWNER);
	...
	case 0xBC:
		...
		{
		case 0x00:
			DATA_SIZE_CHECK(PMSG_REQ_JEWEL_MIX);
			CGReqJewelMix((PMSG_REQ_JEWEL_MIX *)aRecv, aIndex);
			break;
		case 0x01:
			DATA_SIZE_CHECK(PMSG_REQ_JEWEL_UNMIX);
	...
	case 0x73:
		if ( DataEncryptCheck(aIndex, protoNum, Encrypt) != FALSE )
		{
			DATA_SIZE_CHECK(PMSG_NPROTECTGGCHECKSUM);
			GCNPggCheckSumRecv((PMSG_NPROTECTGGCHECKSUM *)aRecv, aIndex);
		}
		break;
	case 0x81:
		DATA_SIZE_CHECK(PMSG_WAREHOUSEMONEYINOUT);
	...
	case 0x82:
		...
		break;
	case 0x83:
		DATA_SIZE_CHECK(PMSG_WAREHOUSEPASSSEND);
		GCWarehouseRecivePassword(aIndex, (PMSG_WAREHOUSEPASSSEND *)aRecv);
		break;
	case 0x86:
		DATA_SIZE_CHECK(PMSG_CHAOSMIX);
	...
	case 0x97:
		...
		break;
	case 0x98:
		DATA_SIZE_CHECK(PMSG_EXCHANGE_EVENTCHIP);
		GCUseRenaChangeZenRecv((PMSG_EXCHANGE_EVENTCHIP *)aRecv, aIndex);
		break;
	case 0x99:
		DATA_SIZE_CHECK(PMSG_REQ_MOVE_OTHERSERVER);
	...
	case 0x9B:
		...
		break;
	case 0x9F:
		DATA_SIZE_CHECK(PMSG_REQ_CL_ENTERCOUNT);
		CGRequestEventEnterCount((PMSG_REQ_CL_ENTERCOUNT *)aRecv, aIndex);
		break;
	case 0x9D:
		DATA_SIZE_CHECK(PMSG_REQ_2ANV_LOTTO_EVENT);
	...
	case 0xAF:
		...
		{
		case 0x01:
			DATA_SIZE_CHECK(PMSG_REQ_MOVECHAOSCASTLE);
			CGRequestEnterChaosCastle((PMSG_REQ_MOVECHAOSCASTLE *)aRecv, aIndex);
			break;
		case 0x02:
			DATA_SIZE_CHECK(PMSG_REQ_REPOSUSER_IN_CC);
	...
	case 0xC9:
		...
		break;
	case 0xCA:
		DATA_SIZE_CHECK(PMSG_FRIEND_ROOMCREATE_REQ);
		FriendChatRoomCreateReq((PMSG_FRIEND_ROOMCREATE_REQ *)aRecv, aIndex);
		break;
	case 0xCB:
		DATA_SIZE_CHECK(PMSG_ROOM_INVITATION);
		FriendRoomInvitationReq((PMSG_ROOM_INVITATION *)aRecv, aIndex);
		break;
	case 0xD0:
		//Season3 Quest MoveGate
		switch (((PMSG_DEFAULT2 *)aRecv)->subcode)
		{
		case 0x07: //WereWolf
			WereWolfQuarrelCheck(aIndex);
			break;
		case 0x08: //GateKeeper
			GateKeeperCheck(aIndex);
			break;
		}
		break;
	case 0xD1:
		...
		{
		case 0x00:
			DATA_SIZE_CHECK(PMSG_REQ_KANTURU_STATE_INFO);
			CGReqKanturuStateInfo((PMSG_REQ_KANTURU_STATE_INFO *)aRecv, aIndex);
			break;
		case 0x01:
			DATA_SIZE_CHECK(PMSG_REQ_ENTER_KANTURU_BOSS_MAP);
...
void GCCheckMainExeKeySend(int aIndex)
{
	...
	PMSG_CHECK_MAINEXE msg(EncryptCheckSumKey(gObj[aIndex].CheckSumTableNum));

	DataSend(aIndex, (UCHAR*)&msg, msg.h.size);
}
...
void PMacProc(PMSG_MAC_ADDR * lpMac, short aIndex)
{
	...
	LogAdd("[MacProc][%s] :: MacAddr : %s",lpObj->AccountID,lpMac->MacAdddr);
}
void PChatProc(PMSG_CHATDATA * lpChat, short aIndex)
{
	...
	switch ( lpChat->chatmsg[0] )
	{
		...
		case '/':	// Command
			if ( slen > 2 )
			{
				strcmp(&lpChat->chatmsg[1], "이동 칼리마");	// #warning Deathway Translation
...
{
	PMSG_JOINRESULT pResult(aIndex,result,szClientVersion);
...
void CGLevelUpPointAdd(PMSG_LVPOINTADD * lpMsg, int aIndex)
{
	...
	LevelUpPointAdd(lpMsg->Type,1,aIndex);
}


void LevelUpPointAdd(byte nType, int nPointCount, int aIndex)
{
	if ( !gObjIsConnected(aIndex))
	{
		CloseClient(aIndex);
	...
	if (::gObjLevelUpPointAdd(nType,nPointCount, &gObj[aIndex]) == TRUE )
	{
		pMsg.ResultType = 0x10;
		pMsg.ResultType += nType;
	...
	for(size_t i=0;i<nPointCount;++i)// add by huihui
	{
		DataSend(aIndex, (UCHAR *)&pMsg, pMsg.h.size);
	}
}
...
void GCMagicListOneSend(int aIndex, char Pos, unsigned char type, unsigned char level, unsigned short skill, BYTE btListType)
{
	...
	PHeadSubSetB((LPBYTE)&pCount, 0xF3, GMSG_MAGIC_LIST, 0);
	pCount.Count = 0xFE;
...
void CGItemGetRequest(PMSG_ITEMGETREQUEST * lpMsg, int aIndex)
{
	...
	if ( lpItem->IsItem() == TRUE && lpItem->Give == false && lpItem->live == true)
	{
		if ( lpItem->m_QuestItem != false )
		{
			...
			if ( ItemCount > 0 && !(lpItem->m_Type >= ITEMGET(14,65) && lpItem->m_Type <= ITEMGET(14,67)) ) //Third Quest Fix
			{
			...
			if ( ItemCount > 2 && (lpItem->m_Type >= ITEMGET(14,65) && lpItem->m_Type <= ITEMGET(14,67)) ) //Third Quest Fix
			{
				LogAdd("[Quest] Too many have quest item [%s][%s] (%s) %d",
					gObj[aIndex].AccountID, gObj[aIndex].Name, lpItem->GetName(), ItemCount);
				pResult.result = -1;
		...
		else
		{
			...
			if ( pResult.result != 0xFF )
			{
				ItemByteConvert((LPBYTE)&pResult.Data[0], lpItem->m_Type, lpItem->m_Option1, lpItem->m_Option2,
					...
					lpItem->m_SetOption, lpItem->m_JewelOfHarmonyOption, lpItem->m_ItemOptionEx, &lpItem->m_ItemSocket[0]);
	...
	if ( pResult.Result == 1 )
	{
		...
		if ( bIsItem == TRUE )
		{
			...
			else if ( type == ITEMGET(14,51) )
			{
				...
				LogAddTD("[%s][%s][Used Star Fireworks Effect] : (%d)(X:%d/Y:%d)",
				lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y);
...
void CGGuildRequestResultRecv(PMSG_GUILDQRESULT * lpMsg, int aIndex) //GS-CS Decompiled 100%
{
	int number;
	BYTE result=0;
	int pnumber=-1;
...
void GCGuildWarRequestResult(LPSTR GuildName, int aIndex, int type)
{
	...
	{
		LogAddC(2,"[ANTI-HACK][Trade & War] (%s)(%s)",
			gObj[aIndex].AccountID, gObj[aIndex].Name);
		return;
	}
...
void CGChaosBoxItemMixButtonClick(PMSG_CHAOSMIX* aRecv, int aIndex)
{
	...
	switch ( iMixType )	// TEST - Good
	{
		...
		case SEED_TYPE_SEEDCALC:
			gItemSocket.SeedCalc(lpObj, aRecv->Type);
			break;
		case SEED_TYPE_NEWMIX2:
			gItemSocket.NewMix4(lpObj, aRecv->Type);
			break;
		default:
...
void PMoveProc(PMSG_MOVE* lpMove, int aIndex)
{
		...
		switch ( g_BloodCastle.GetCurrentState(lpObj->MapNumber-11) )
		{
			case 0x01:
				if ( lpObj->TY > 15 )
				{
					...
					PHeadSetB((LPBYTE)&pActionResult, GMSG_ACTION, sizeof(pActionResult));
	...
	PHeadSetB((LPBYTE)&pMove, GMSG_MOVE, sizeof(pMove));
...
void RecvPositionSetProc(PMSG_POSISTION_SET * lpMove, int aIndex)
{
	...
	PHeadSetB((LPBYTE)&pMove, GMSG_POSISTION_SET, sizeof(pMove));
	pMove.NumberH = SET_NUMBERH(aIndex);
	pMove.NumberL = SET_NUMBERL(aIndex);
	pMove.X = lpMove->X;
	pMove.Y = lpMove->Y;
...
void GCDamageSend(int aIndex, int TargetIndex, int AttackDamage, int MSBFlag, int MSBDamage, int iShieldDamage)
{
	...
	PHeadSetB((LPBYTE)&pResult, GMSG_ATTACK, sizeof(pResult));
...
void GCKillPlayerExpSend(int aIndex, int TargetIndex, int exp, int AttackDamage, BOOL MSBFlag)
{
	...
	pkillMsg.h.setE(0x16, sizeof(pkillMsg));
...
void CGActionRecv(PMSG_ACTION * lpMsg, int aIndex)
{
	...
	PHeadSetB((LPBYTE)&pResult, GMSG_ACTION, sizeof(pResult));
...
void CGMagicAttack(PMSG_MAGICATTACK* lpMsg, int aIndex)
{
	...
	int nMagicNumber = lpMsg->GetMagicNumber();
	if ( lpObj->Type == OBJ_USER )
	{
		if ( nMagicNumber == 58 )
			usernumber = 58;	// #error BIG ERROR - Deathway

		lpMagic = gObjGetMagicSearch(lpObj, nMagicNumber);
	...
	{
		lpMagic = gObjGetMagic(lpObj, nMagicNumber);
...
{
	PMSG_MAGICATTACK_RESULT pAttack(MagicNumber,lpObj->m_Index,usernumber);
...
{
	PMSG_MAGICCANCEL pMsg(MagicNumber,lpObj->m_Index);
...
{
	PMSG_VIEWSKILLSTATE pMsg(lpObj->m_Index,state,ViewSkillState>>24);
...
void CGTeleportRecv(PMSG_TELEPORT* lpMsg, int aIndex)
{
	...
	if ( lpMsg->MoveNumber == 0 )
	{
		...
		BYTE x;
		BYTE y;
		PMSG_MAGICATTACK_RESULT pAttack(6,aIndex,aIndex);
		...
		if ( gObjCheckTeleportArea(aIndex, x, y) == FALSE )
		{
			...
			PMSG_TELEPORT_RESULT pTeleportResult(aIndex,0,gObj[aIndex].MapNumber,gObj[aIndex].X,gObj[aIndex].Y,gObj[aIndex].Dir);
...
void CGTargetTeleportRecv(PMSG_TARGET_TELEPORT * lpMsg, int aIndex)
{
	...
	PMSG_MAGICATTACK_RESULT pAttack(AT_SKILL_TARGET_TELEPORT,aIndex,iTargetIndex);
...
void GCTeleportSend(LPOBJ lpObj, BYTE MoveNumber, BYTE MapNumber, BYTE MapX, BYTE MapY, BYTE Dir)
{
	...
	PMSG_TELEPORT_RESULT pMsg(0,MoveNumber,MapNumber,MapX,MapY,Dir);
	DataSend(lpObj->m_Index, (UCHAR*)&pMsg, pMsg.h.size);
}
...
void CGBeattackRecv(unsigned char* lpRecv, int aIndex, int magic_send)
{
	...
	// Check the Protocol
	if ( lpCount->h.headcode != GMSG_BEATTACK )
	{
		LogAdd("error-L3 %s %d", __FILE__, __LINE__);
		return;
	}
	...
	int nMagicNumber = lpCount->GetMagicNumber();
	// Set lpMagic according
	// if aIndex is OBJ_USER
	// or if it is OBJ_MONSTER
	if ( gObj[aIndex].Type == OBJ_USER )
	{
		lpMagic = gObjGetMagicSearch(&gObj[aIndex], nMagicNumber);
	}
	else
	{
		lpMagic = gObjGetMagic(&gObj[aIndex], nMagicNumber);
	}
	...
	for (int n=0;n<lpCount->Count;n++)
	{
		...
		if ( tNumber >= 0 && tNumber < OBJMAX-1 )
		{
			...
			{
				gObjUseSkill.SkillFireScream(aIndex, tNumber, lpMagic); // no all that what i showed u
			}
			else if ( magic_send )
			{
				gObjAttack(&gObj[aIndex], &gObj[tNumber], lpMagic, TRUE, 1, 0, FALSE);
...
{
	PMSG_REFILL pMsg(Ipos,Life,wShield);
	DataSend(aIndex, (UCHAR*)&pMsg, pMsg.h.size);
}
...
void GCManaSend(int aIndex, short Mana, BYTE Ipos, unsigned char flag,  WORD BP)
{
	...
	PMSG_MANASEND pMsg(Ipos,Mana,BP);
	DataSend(aIndex, (UCHAR*)&pMsg, pMsg.h.size);
}
...
void GCSendQuestInfo(int aIndex, int QuestIndex)
{
	...
	if(QuestIndex == 5 || QuestIndex == 6)
	{
		...
		DataSend(aIndex,QuestCount,QuestCount[1]);
	}
...
void GCAnsCastleSiegeState(int iIndex, int iResult, LPSTR lpszGuildName, LPSTR lpszGuildMaster)
{
	...
	#pragma message(" [INFO] Prodito : I don't know if the function is correct")
	...
	// Time Left
	#pragma message(" [INFO] Prodito : Check this")
...
void CGReqRegGuildMark(PMSG_REQ_REGGUILDMARK * lpMsg, int iIndex)
{
	...
	#pragma message(" [INFO] Prodito : There Should be a define, but i don't found one ")
	...
	else
	{
		...
		{
			if( gObj[iIndex].pInventory[iItemPos].m_Type == ITEMGET(0,469) )
			{
				if( gObj[iIndex].pInventory[iItemPos].m_Level == 3 )
				{
					if( ((gObj[iIndex].pInventory[iItemPos].m_Durability - 1.0 ) - gObj[iIndex].pInventory[iItemPos].m_Durability) > 0.0 ) // Prodito : Review
					{
						PMSG_ANS_REGGUILDMARK pMsgResult = {0};
						pMsgResult.h.set(0xB2, 0x04, sizeof(pMsgResult));
						pMsgResult.btResult = 3;		
					...
					case 1:
						{
							...
							//	iNEED_GEMOFDEFEND = ((iNXT_UPLVL-1) * 12) + g_iGemOfDefendRate;
							//	iNEED_MONEY = ((iNXT_UPLVL-1) * 12) + g_iGemOfDefenseZenRate;
							}
						}
						break;
...
//									iNXT_UPLVL = (i+1);
							...
							{
//								bENABLE_UPGRADE = TRUE;
//								iNXT_UPVALUE = g_iNpcUpMaxHP_CGATE[iNXT_UPLVL-1];
								#pragma message("[Prodito] [INFO] : I need to test source, i don't know if it fine, so i stop here.")
								// 0044E603  :: mov eax, [local.iNXT_UPLVL]
							}
						}
						break;
					}
...
void GCAnsMoneyDrawOut(int iIndex, int iResult, __int64 i64Money)
{
	...
	pMsgResult.btResult = iResult;
	pMsgResult.btMoney1 = SET_NUMBERH(SET_NUMBERHW(HIDWORD(i64Money)));//100%
	pMsgResult.btMoney2 = SET_NUMBERL(SET_NUMBERHW(HIDWORD(i64Money)));//100%
	pMsgResult.btMoney3 = SET_NUMBERH(SET_NUMBERLW(HIDWORD(i64Money)));//100%
	pMsgResult.btMoney4 = SET_NUMBERL(SET_NUMBERLW(HIDWORD(i64Money)));//100%
...
void GCMsgTest(int aIndex, char* Message, unsigned char* Photo, unsigned char Dir, unsigned char Action)
{
	...
	pMsg.MemoIndex = aIndex;
	pMsg.MemoSize = strlen(Message);
	pMsg.Dir = 1;
	pMsg.Number -1;
	memcpy(pMsg.Name,"lol",3);
	pMsg.Action = 1;
	//memcpy(pMsg.Photo, lpMsg->Photo,sizeof(pMsg.Photo));
	memset(pMsg.Photo , 1, 18);
	memcpy(pMsg.Memo, Message, strlen(Message));
...
void WereWolfQuarrelCheck(int aIndex)
{
	...
	{
	gObj[aIndex].Money -= 3000000; //3Kk
	GCMoneySend(gObj[aIndex].m_Index, gObj[aIndex].Money);
	gObjMoveGate(aIndex, 240); //Balgass Barracks Gate
	}
	else
	{
		LogAddTD("[Anti-Hack][WereWolf Quarrel] [%d][%d] User try to Enter Barracks without Season3 Quest State",gObj[aIndex].AccountID, gObj[aIndex].Name);
	}
...
void GateKeeperCheck(int aIndex)
{
	...
	{
	gObj[aIndex].Money -= 5000000; //5Kk
	GCMoneySend(gObj[aIndex].m_Index, gObj[aIndex].Money);
	gObjMoveGate(aIndex, 241); //Refuge of Balgass Gate
	}
	else
	{
		LogAddTD("[Anti-Hack][GateKeeper] [%d][%d] User try to Enter Refugee without Season3 Quest State",gObj[aIndex].AccountID, gObj[aIndex].Name);
	}
}
...
void GCSendObjectCreationState(int iObjectIndex)
{
	...
	if(lpObj->Class == 278)
	{
		PHeadSubSetB((LPBYTE)&pMsg,0xb9,0x01,sizeof(PMSG_NOTIFY_OBJECT_CREATION_STATE));
		...
		::MsgSendV2(lpObj,(LPBYTE)&pMsg,sizeof(PMSG_NOTIFY_OBJECT_CREATION_STATE));
	}
}
