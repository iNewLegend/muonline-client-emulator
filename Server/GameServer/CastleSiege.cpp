...
int CCastleSiege::LoadData(char * lpszFileName)
{
	...
	_CS_NPC_DATA * it2 = m_vtNpcData.begin();
...
void CCastleSiege::CreateAllCsGateLever()
{
	...
	{
		for(_CS_NPC_DATA * it = m_vtNpcData.begin(); it != m_vtNpcData.end(); it++)
		{
...
void CCastleSiege::CreateDbNPC()
{
	...
	_CS_NPC_DATA * it = m_vtNpcData.begin();
	int bExist = 0;
...
void CCastleSiege::ReSpawnAllUser()
{
	...
	for( int iUSER = OBJ_STARTUSERINDEX; iUSER < OBJMAX; iUSER++ )
	{
		...
		if( bMoveOK )
		{
#pragma message(" [FIXME] Prodito : Webzen fault, change TX to TY!")
...
void CCastleSiege::SaveCsTotalGuildInfo()
{
	...
	for( map<basic_string<char>, _CS_TOTAL_GUILD_DATA>::iterator it = (map<basic_string<char>, _CS_TOTAL_GUILD_DATA>::iterator)m_mapCsTotalGuildInfo.begin(); it != m_mapCsTotalGuildInfo.end(); it++)
	{
		if( lpMsgSend->iCount > 150 )
		{
			break;
