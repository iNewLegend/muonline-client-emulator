...
int gObjViewSkillBufferConvert(LPOBJ lpObj, LPBYTE lpViewSkillBuffer)
{
	if ( lpViewSkillBuffer == NULL )
		return 0;

	int iCount = 0;	// Count of How much ViewSkillBuffer found
...
/*	lpObj->q_QuestIndex = 0;
	lpObj->q_QuestState = 0;
	lpObj->q_MangerIndex = 0;
	lpObj->q_QuestSubIndex = 0;*/
...
void DbItemSetInByte(LPOBJ lpObj, struct SDHP_DBCHAR_INFORESULT* lpMsg, int ItemDbByte, bool* bAllItemExist)
{
	...
	for(int n = 0; n < INVENTORY_SIZE;n++)
	{
		...
		if(itype != -1)
		{
			...
			{ // VER DEPOIS
				lpObj->pInventory[n].m_ItemSocket[it] = item.m_ItemSocket[it];
			}
...
void gObjSetBP(int aIndex)
{
	...
	switch ( gObj[aIndex].Class )
	{
		...
		case CLASS_SUMMONER :
			gObj[aIndex].MaxBP = (Strength * 0.2) + (Dexterity * 0.25) + (Vitality * 0.3) + (Energy * 0.15); // Verificar Valores
			break;
	}
}
...
BOOL gObjSetCharacter(LPBYTE lpdata, int aIndex)
{
	...
	if ( lpObj->Level < 6 || DS_MAP_RANGE(lpObj->MapNumber) != FALSE )
	{
		...
		{
			lpObj->MapNumber = MAP_INDEX_MARKET;
			MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber, lpObj->X, lpObj->Y);
		}
		else
		{
	...
	if ( lpObj->Class == 0 || lpObj->Class == 2 || lpObj->Class == 1 || lpObj->Class == 4 || lpObj->Class == 3 || lpObj->Class == 5 )
	{
		...
		if ( lpObj->Class == 5 )	// Freiya
		{
			if ( bEnergyBall == FALSE )
			{
				gObjMagicAdd(lpObj, 17, 0);
				bEnergyBall = TRUE;
			}
...
BOOL gObjBackSpring(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	pMove.h.c = 0xC1;
	pMove.h.headcode = GMSG_POSISTION_SET;
...
bool gObjLevelUp(LPOBJ lpObj, int addexp, int iMonsterType, int iEventType)
{
	...
	if ( lpObj->Class == 4 )
	{
		lpObj->LevelUpPoint += g_ServerValues.g_MGDLLevelUpPoints;
	}
	...
	{
		lpObj->LevelUpPoint += g_ServerValues.g_NormalLevelUpPoints;
	}

	if ( lpObj->PlusStatQuestClear != false )
	{
		lpObj->LevelUpPoint += g_ServerValues.g_QuestAddLevelUpPoints;
	...
	g_Manger.OnLevelUp(lpObj->m_Index);
...
BOOL gObjLevelUpPointAdd(BYTE type, unsigned int uPointCount, LPOBJ lpObj)	// type : [1:Strength][2:Agility][3:Vitality][4:Energy][5:Leadership]
{
	...
	if ( lpObj->LevelUpPoint < uPointCount )
	{
		return FALSE;
	}
	...
	{
		case 0:
			if(lpObj->Strength+uPointCount > 32766)
			{
				//GCServerMsgStringSend("You Can't add moer then 32766",lpObj->m_Index,0);
				return false;
			}
			lpObj->Strength+=uPointCount;
			break;

		case 1:
			if(lpObj->Dexterity+uPointCount > 32766)
			{
				//GCServerMsgStringSend("You Can't add moer then 32766",lpObj->m_Index,0);
				return false;
			}
			lpObj->Dexterity+=uPointCount;
			break;

		case 2:
			if(lpObj->Vitality+uPointCount > 32766)
			{
				//GCServerMsgStringSend("You Can't add moer then 32766",lpObj->m_Index,0);
				return false;
			}
			lpObj->Vitality+=uPointCount;

			lpObj->MaxLife += lpObj->VitalityToLife*uPointCount;
			gObjCalcMaxLifePower(lpObj->m_Index);
			break;

		case 3:
			if(lpObj->Energy+uPointCount > 32766)
			{
				//GCServerMsgStringSend("You Can't add moer then 32766",lpObj->m_Index,0);
				return false;
			}
			lpObj->Energy+=uPointCount;
			lpObj->MaxMana += lpObj->EnergyToMana*uPointCount;
			break;

		case 4:
			if(lpObj->Leadership+uPointCount > 32766)
			{
				//GCServerMsgStringSend("You Can't add moer then 32766",lpObj->m_Index,0);
				return false;
			}
			lpObj->Leadership+=uPointCount;
			break;
	}

	lpObj->LevelUpPoint-=uPointCount;
	gObjCalCharacter(lpObj->m_Index);
...
void gObjUserDie(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	if(gPkLimitFree == 0)
	{
		...
		{
			//itemdrop = rand()%32;
			LogAddTD("[PK DROP SYSTEM] itemdrop = Applied fix on kill");
		}
		else if(lpObj->m_PK_Level == 3)
		{
			//itemdrop = rand()%16;
	...
	else
	{
		...
		{
			//itemdrop = rand()%128;
			LogAddTD("[PK DROP SYSTEM] Applied fix on kill");
		}
		else if(lpObj->m_PK_Level == 3)
		{
			//itemdrop = rand()%64;
...
void gObjExpParty(LPOBJ lpObj , LPOBJ lpTargetObj, int AttackDamage, int MSBFlag )
{
	...
	if(viewplayer > 1)
	{
		...
		{
			if(viewplayer == 3) //here is the config ..
			{
				viewpercent = 200;
				//viewpercent = 200;
			}
	...
	if(lpTargetObj->Type == OBJ_MONSTER)
	{
		lpTargetObj->Money = totalexp * gAddExperience; // Zen Value To Drop :)
	}
	...
	for(int n = 0; n < MAX_USER_IN_PARTY; n++)
	{
		...
		if(number >= 0)
		{
			...
			if(lpTargetObj->MapNumber == lpPartyObj->MapNumber && dis[n] < 10)
			{
				...
				exp = (totalexp * viewpercent * lpPartyObj->Level)/totallevel / 100 * 2; // GENERAL PARTY EXP
	...
	if(lpTargetObj->Life <= 0 && lpTargetObj->Live != 0)
	{
		...
		if(lpTargetObj->Type == OBJ_MONSTER)
		{
			...
			if( lpTargetObj->m_Attribute == 0x3E )
			{
				...
				if( iTargetClass != 295)
				{
					if( iTargetClass > 299 && 303 < iTargetClass )
					{
					...
					else
					{
						ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, lpObj->X, lpObj->Y,
							ItemGetNumberMake(14, 13), 0, 0, 0, 0, 0, lpObj->m_Index, 0, 0);

						if( gObjIsConnected(lpObj->m_Index) )
						{
							wsprintf(szBuff, lMsg.Get(MSGGET(12,155)), lpTargetObj->Name, lpObj->Name);
							
							LogAddTD("[CastleDeep Event] [%s][%s] Boss Monster Killed : %s",
								lpObj->AccountID, lpObj->Name, lpTargetObj->Name);
						}
						else
						{
							wsprintf(szBuff, lMsg.Get(MSGGET(12,156)), lpTargetObj->Name);
				...
				else
				{
					if( gObjIsConnected(lpObj->m_Index) )
					{	
						wsprintf(szBuff, lMsg.Get(MSGGET(12,157)), lpObj->Name);
		...
		if(lpCallObj->Type == OBJ_USER)
		{
			...
			{
				int partycount = gParty.m_PartyS[lpCallObj->PartyNumber].Count;

				LogAdd("partycount %d",partycount);

				if( partycount == 2)
				{
					int number = gParty.m_PartyS[lpCallObj->PartyNumber].Number[1];
					LPOBJ userlox = &gObj[number];
					
					int uDas = gObjCalDistance(lpTargetObj,userlox);
					LogAdd("LpTarget %dX%d",lpTargetObj->X,lpTargetObj->Y);
					LogAdd("userlox %dX%d",userlox->X,userlox->Y);
					LogAdd("Distance %d",uDas);
					if(uDas > 10)
					{
						LogAdd("Party Distance too much for get exp");
						//goto FUCKASS;
					}
				}
...
BOOL gObjIsItemPut(LPOBJ lpObj, CItem * lpItem, int pos )
{
	...
	if(lpItem->m_Type == ITEMGET(13,10))
	{
		...
		count1 = gObjGetItemCountInEquipment(lpObj->m_Index,13,40,0);
			...
			if(sFlag == 0)
			{
				if(tFlag == 5)
				{
					if(sitem->m_Type == ITEMGET(12,15)	||
						...
						sitem->m_Type == ITEMGET(14,16) || // LIFE
				...
				else
				{
					...
					{
						if(sitem->m_Type == ITEMGET(14,16) || sitem->m_Type == ITEMGET(12,15) || sitem->m_Type == ITEMGET(14,13) || sitem->m_Type == ITEMGET(14,14) || sitem->m_Type == ITEMGET(14,22) || sitem->m_Type == ITEMGET(13,14))
						{
						...
						else if(sitem->m_Type == ITEMGET(12,26))
						{
							...
							sitem->m_Type == ITEMGET(13,49) ||
							sitem->m_Type == ITEMGET(13,50) ||
						...
						else if(sitem->m_Type == ITEMGET(13,49))
						{

						}
						else if(sitem->m_Type == ITEMGET(13,50))
						{
...
void gObjMakePreviewCharSet(int aIndex)
{
	...
	if ( (lpObj->pInventory[7].m_Type >= ITEMGET(12,3) && lpObj->pInventory[7].m_Type <= ITEMGET(12,6)) ||
		  lpObj->pInventory[7].m_Type == ITEMGET(13,30) || (lpObj->pInventory[7].m_Type >= ITEMGET(12,36) && lpObj->pInventory[7].m_Type <= ITEMGET(12,40)) )
	{
...
*/
	...
	if (lpObj->pInventory[7].m_Type == ITEMGET(12,40))
		{
			lpObj->CharSet[16] |= 20;
		}
...
void gObjViewportListProtocolCreate(LPOBJ lpObj)
{
	...
	BYTE btViewSkillState[MAX_VIEWSKILL] = {0};
	...
	else
	{
		...
		if(moncount > 0)
		{
			...
			pwCount.h.c = 0xC2;
			pwCount.h.headcode = GMSG_MONSTER_VIEWPORT_CREATE;
			pwCount.h.sizeH = SET_NUMBERH(lOfs);
			pwCount.h.sizeL = SET_NUMBERL(lOfs);
...
void gObjViewportListProtocol(short aIndex)
{
	...
	count = 0;
	count_Item = 0;
	BYTE btViewSkillBuffer[MAX_VIEWSKILL] ={0};
	...
	if(lpObj->Type == OBJ_USER)
	{
		...
		{
			pCount.h.c = 0xC1;
			pCount.h.headcode = GMSG_VIEWPORT_DESTROY;
			pCount.h.size = lOfs;
			pCount.count = count;
	...
	else
	{
					...
					if(tObjNum >= 0)
					{
						...
						case 1:
							...
							//	lpTargetObj->CharSet[0] &= 0xF0;
								...
								//pViewportCreateChange.ViewSkillStateCount = lpTargetObj->m_ViewSkillState;
								pViewportCreateChange.DirAndPkLevel = lpTargetObj->Dir << 4;
								pViewportCreateChange.DirAndPkLevel |= lpTargetObj->m_PK_Level & 0x0F;

								// aqui ele poe as modificacoes coloca isso lah em baixo
								...
								if( g_CastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE )
								{
									...
									case 1:
										...
										// isso aqui muda o estado do barato no sistema novo... ai a conversao vc faz depois so
							...
							//	memcpy(pViewportCreateChange.CharSet,lpTargetObj->CharSet,sizeof(pViewportCreateChange.CharSet)); //New for Fix Rings of Transformation // huihu inot sure
							...
							else
							{
								...
								//pViewportCreate.ViewSkillStateCount = lpTargetObj->m_ViewSkillState;
						...
						case 3:
							if(lpObj->Type == OBJ_USER)
							{
								...
								else
								{
									...
									//0050BFD4  |.  3D D8000000   |cmp eax, 0D8
		...
		if(lpObj->Type == OBJ_USER)
		{
			...
			if(monstercount > 0)
			{
				...
				pCount.h.c = 0xC2;
				pCount.h.headcode = GMSG_MONSTER_VIEWPORT_CREATE;
...
void gObjSkillUseProc(LPOBJ lpObj)
{
	...
	if(lpObj->Class == 77) // have fun !
	{
		...
		if(lpObj->m_SkyBossMonSheildTime > 5)
		{
			...
			{
				lpObj->m_SkyBossMonSheild = 0;
				lpObj->m_ViewSkillState[eVS_INMUNE_TO_MAGIC] = 0;
...
struct PMSG_POSMAGIC_COUNT {
  ...
  /*<thisrel this+0x3>*/ /*|0x1|*/ unsigned char NumberH;
  /*<thisrel this+0x4>*/ /*|0x1|*/ unsigned char NumberL;
  /*<thisrel this+0x5>*/ /*|0x1|*/ unsigned char MagicNumberH;
  /*<thisrel this+0x5>*/ /*|0x1|*/ unsigned char MagicNumberL;
...
BOOL gObjItemRandomLevelUp(LPOBJ lpObj, int source, int target)
{
	...
	if(lpObj->pInventory[target].m_Option2) // With Luck
	{
		if(_r < g_ServerRates.g_SoulWithLuckRate ||  g_ServerRates.g_SoulWithLuckRate == 0)
		{
			lpObj->pInventory[target].m_Level++;
		}
		else
	...
	{
		if(_r < g_ServerRates.g_SoulWithOutLuckRate) // Without luck
		{
			if(lpObj->pInventory[target].m_Level >= 7)
			{
				lpObj->pInventory[target].m_Level = 0;
...
BOOL gObjItemRandomOption3Up(LPOBJ lpObj, int source, int target)
{
	...
	if(lpObj->pInventory[target].m_Option3 == 0)
	{
		...
		{
			lpObj->pInventory[target].m_NewOption &= 0xDF;// GMSG_POSISTION_SET?
	...
	if(lpObj->pInventory[target].m_Option3 < 4) // Orginal 4
	{
		if(_r < g_ServerRates.g_LifeRate) // ORGINAL 50
		{
...
void gObjSendUserStatistic(int aIndex, int startLevel, int endLevel)
{
	...
	MsgOutput(gObj[aIndex].m_Index,"[%d]-[%d] Total %d %s %d %s %d %s %d %s %d",startLevel,endLevel,sClassCount[4],
		...
		lMsg.Get(1903),sClassCount[CLASS_SUMMONER]);
}
...
int gObjGetIndexByName(char* Name)
{
	for (int i = OBJ_STARTUSERINDEX ; i<OBJMAX;i++)
	{
		...
		{
			if (!strcmp(gObj[i].Name, Name)) return i;
		}
	}
	return 0;
}
...
void gObjGetStatPointState(int aIndex, short& AddPoint, short& MaxAddPoint, short& MinusPoint, short& MaxMinusPoint)
{
	...
	{
		ori_point = DCInfo.GetDefPoint(lpObj->Class) + (lpObj->Level - 1) * 6;
	}
...
struct PMSG_WEAPON_OWNER_VIEWPORT_NOTIFY {
  ...
  /*<thisrel this+0x18>*/ /*|0x1|*/ unsigned char btViewSkillCount;
...
//#pragma message("[DW] Fix this PArt of View Skill")
...
LPSTR gObjGetClassName(int classid)
{
