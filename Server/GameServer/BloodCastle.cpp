...
void CBloodCastle::Init(bool bEVENT_ENABLE)
{
	...
	for ( int n =0; n<gMSetBase.m_Count;n++)
	{
		if ( BC_MAP_RANGE(gMSetBase.m_Mp[n].m_MapNumber ) != FALSE )
		{
			...
			{
				this->m_BridgeData[gMSetBase.m_Mp[n].m_MapNumber - MAP_INDEX_BLOODCASTLE1].m_iAngelKingPosNum = n;
				continue;
			}
...
void CBloodCastle::SetState_Closed(int iBridgeIndex)
{
	...
	for (int n=OBJ_STARTUSERINDEX;n<OBJMAX;n++)
	{
		if ( gObj[n].MapNumber == iBridgeIndex + 11 && gObj[n].Connected > PLAYER_LOGGED )
		{
...
int  CBloodCastle::CheckEnterLevel(int iIndex, int iLevel)	// RET : [2:Error][1:OverLevel][0:InLevel - Success][-1:UnderLevel]
{
	...
	if ( gObj[iIndex].Class == CLASS_DARKLORD || gObj[iIndex].Class == CLASS_MAGUMSA || gObj[iIndex].Class == CLASS_SUMMONER )
	{
...
bool CBloodCastle::BloodCastleChaosMix(int iIndex, int iLEVEL)
{
	...
	LogChaosItem(&gObj[iIndex], "Åõ¸í¸ÁÅä Mix");
	...
	if ( (rand()%100) < iMIX_SUCCESS_RATE )	// Siccess
	{
		...
		LogAddTD("[Åõ¸í¸ÁÅä Mix] [%s][%s] CBMix Success %d Money : %d-%d",
	...
	else	// Failure
	{
		...
		DataSend(iIndex, (UCHAR*)&pMsg, pMsg.h.size);
		LogAddTD("[Åõ¸í¸ÁÅä Mix] [%s][%s] CBMix Fail %d Money : %d-%d",
			gObj[iIndex].AccountID, gObj[iIndex].Name, iMIX_SUCCESS_RATE, gObj[iIndex].Money, iMIX_NEED_MONEY);
...
//APLICAR MINHA FUNÇÃO ESPECIAL AQUI *****
	if ( this->GetCurrentState(gObj[iIndex].MapNumber - MAP_INDEX_BLOODCASTLE1 ) == TRUE )	// Apply Deathway fix here : Add -MAP_INDEX_BLOODCASTLE1
	{
...
void CBloodCastle::ClearMonster(int iBridgeIndex, bool bClearCastleDoor)
{
	...
	{
		if ( gObj[n].MapNumber == (iBridgeIndex+MAP_INDEX_BLOODCASTLE1))
		{
...
void CBloodCastle::SetMonster(int iBridgeIndex)
{
	...
	for ( int n=0;n<gMSetBase.m_Count;n++)
	{
		if ( BC_MAP_RANGE(gMSetBase.m_Mp[n].m_MapNumber) != FALSE  )
		{
			BYTE btIndex = gMSetBase.m_Mp[n].m_Type;
...
void CBloodCastle::SetBossMonster(int iBridgeIndex)
{
	...
	for ( int x=0;x<MAX_BLOOD_CASTLE_BOSS_MONSTER;x++)
	{
		if ( this->m_BridgeData[iBridgeIndex].m_nBossMonsterPosNum[x] != -1 )
		{
			...
			BYTE btMonsterType = gMSetBase.m_Mp[nBossMonsterPosNum].m_Type;
...
int  CBloodCastle::LevelUp(int iIndex, int iAddExp, int iEventType)
{
	...
	LogAddTD("Experience : Map[%d]-(%d,%d) [%s][%s](%d) %u %d MonsterIndex : %d, EventType : %d",
		gObj[iIndex].MapNumber, gObj[iIndex].X, gObj[iIndex].X,	// #error gObj[iIndex].X -> gObj[iIndex].Y
	...
	else
	{
		...
		if ( gObj[iIndex].Class == CLASS_SUMMONER )
		{
		...
		{
			gObj[iIndex].LevelUpPoint += 8;
		}
		else
		{
			gObj[iIndex].LevelUpPoint += 6;
		}
...
void CBloodCastle::CheckUsersOnConnect(int iBridgeIndex)
{
	...
	for ( int i=0;i<MAX_BLOOD_CASTLE_SUB_BRIDGE;i++)
	{
		if ( this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex != -1 )
		{
			...
			{
				if ( gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].MapNumber != (iBridgeIndex + MAP_INDEX_BLOODCASTLE1) )
				{
					this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex = -1;
				}
			}
...
void CBloodCastle::DropChaosGem(int iIndex)
{
	...
	int iType = ::ItemGetNumberMake(12, 32);	// BC REWARD
	ItemSerialCreateSend(gObj[iIndex].m_Index, gObj[iIndex].MapNumber + 227, gObj[iIndex].X,
		gObj[iIndex].Y, iType, 0, 0, 0, 0, 0, iMaxHitUser, 0, 0);
}
...
bool CBloodCastle::CheckAngelKingExist(int iBridgeIndex)
{
	...
	if ( bRET_VAL == false )
	{
		int result = gObjAddMonster(iBridgeIndex+MAP_INDEX_BLOODCASTLE1);
...
{
	if ( BC_MAP_RANGE(iBridgeIndex+MAP_INDEX_BLOODCASTLE1) == FALSE )
	{
