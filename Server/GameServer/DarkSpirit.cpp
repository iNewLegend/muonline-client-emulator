...
void CDarkSpirit::Set(int aIndex, CItem * pPetItem)
{
	...
	this->m_AttackDamageMin = petitemlevel*30 + leadership/2 + 180;
	this->m_AttackDamageMax = petitemlevel*30 + leadership/1 + 200;
	...
	this->m_SuccessAttackRate= petitemlevel + petitemlevel / 2 + 9999000;
...
BOOL CDarkSpirit::Attack(LPOBJ lpObj, LPOBJ lpTargetObj, CMagicInf * lpMagic, int criticaldamage, int iActionType)
{
	...
	lpObj->pInventory[1].m_DurabilitySmall += AttackDamage * 2 / 1000; //Dark Raven Life
	...
	if ( gObjWingSprite(lpTargetObj) == TRUE )
	{
		...
		if ( AttackDamage > 1 )
		{
			...
			else if ( Wing->m_Type > ITEMGET(12, 35) && Wing->m_Type < ITEMGET(12, 40) ) //Third Wings Damage Absortion
			{
				float damage = (float)(AttackDamage * (61 - (Wing->m_Level*2))) / 100.0f;
				AttackDamage = (int)(damage);	//  #formula
			}
			else if ( Wing->m_Type == ITEMGET(12, 40) ) //Third Wings Damage Absortion
			{
	...
	{
		// Leo123 Reflect Rate
		if(Role(0,100) <= g_ServerValues.g_DarkSpirtReflect)
		{
...
BOOL CDarkSpirit::MissCheckPvP(LPOBJ lpObj, LPOBJ lpTargetObj, int skill,  int skillSuccess, BOOL& bAllMiss)
{
	...
	// VERIFICAR
	if ( (int)dwRate > iAttackSuccessRate )
	{
