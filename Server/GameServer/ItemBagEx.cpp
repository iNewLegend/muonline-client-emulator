...
BYTE CItemBagEx::GetLevel(int n)
{
	...
	int level = this->BagObject[n].m_minLevel;
...
BOOL CItemBagEx::DropGMBoxEventItem(int aIndex)
{
	...
	if ( this->GetBagCount() > 0 )
	{
		DropItemNum = rand() % this->GetBagCount();
			...
			LogAddTD("[GM BOX] Event ItemDrop [%s][%s] [%s] : (%d)(%d/%d) Item:(%s)%d Level:%d op1:%d op2:%d op3:%d",
				lpObj->AccountID, lpObj->Name, this->m_sEventName,
				lpObj->MapNumber, x, y, ItemAttribute[type].Name,
				type, level, Option1, Option2, Option3);
...
BOOL CItemBagEx::DropBigDragonItem(int aIndex, BYTE btMapNumber, BYTE cX, BYTE cY)
{
	if ( this->m_bLoad == FALSE )
		return FALSE;
	...
	if ( this->GetBagCount() > 0 )
	{
		...
		if ( iDropItemRate < g_iBigDragonDropZenRate ) 
		{
			MapC[lpObj->MapNumber].MoneyItemDrop(g_iBigDragonDropZen, cX, cY);

			LogAddTD("[ BigDragon ][Reward] BigDragon Item Drop [%s][%s] [%d Zen]",
				lpObj->AccountID, lpObj->Name, g_iBigDragonDropZen);
		}
		else
		{
			iDropItemNum = rand()% this->GetBagCount();
			...
			LogAddTD("[ BigDragon ][Reward] BigDragon Item Drop [%s][%s] : (%d)(%d/%d) Item:(%s)%d Level:%d op1:%d op2:%d op3:%d ExOp:%d",
				lpObj->AccountID, lpObj->Name, btMapNumber, X, Y, ItemAttribute[iType].Name,
				iType, iLevel, iOption1, iOption2, iOption3, iExOption);
...
BOOL CItemBagEx::DropZaikanItem(int aIndex, BYTE btMapNumber, BYTE cX, BYTE cY)
{
	if ( this->m_bLoad == FALSE )
		return FALSE;
	...
	if ( this->GetBagCount() > 0 )
	{
		...
		if ( iDropItemRate < g_iZaikanDropZenRate ) 
		{
			MapC[lpObj->MapNumber].MoneyItemDrop(g_iBigDragonDropZen, cX, cY);

			LogAddTD("[ Zaikan ] Got killed by and got [%s][%s] [%d Zen]",
				lpObj->AccountID, lpObj->Name, g_iZaikanDropZen);
		}
		else
		{
			iDropItemNum = rand()% this->GetBagCount();
			...
			LogAddTD("[ Zaikan ] Was Killed and The Gift Item Is [%s][%s] : (%d)(%d/%d) Item:(%s)%d Level:%d op1:%d op2:%d op3:%d ExOp:%d",
				lpObj->AccountID, lpObj->Name, btMapNumber, X, Y, ItemAttribute[iType].Name,
				iType, iLevel, iOption1, iOption2, iOption3, iExOption);
...
BOOL CItemBagEx::DropBigBaliItem(int aIndex, BYTE btMapNumber, BYTE cX, BYTE cY)
{
	if ( this->m_bLoad == FALSE )
		return FALSE;
	...
	if ( this->GetBagCount() > 0 )
	{
		...
		if ( iDropItemRate < g_iBigBaliDropZenRate ) 
		{
			MapC[lpObj->MapNumber].MoneyItemDrop(g_iBigBaliDropZen, cX, cY);

			LogAddTD("[ Bali ][Reward] Bali Item Drop [%s][%s] [%d Zen]",
				lpObj->AccountID, lpObj->Name, g_iBigBaliDropZen);
		}
		else
		{
			iDropItemNum = rand()% this->GetBagCount();
			...
			LogAddTD("[ Bali ][Reward] Bali Item Drop [%s][%s] : (%d)(%d/%d) Item:(%s)%d Level:%d op1:%d op2:%d op3:%d ExOp:%d",
				lpObj->AccountID, lpObj->Name, btMapNumber, X, Y, ItemAttribute[iType].Name,
				iType, iLevel, iOption1, iOption2, iOption3, iExOption);
...
BOOL CItemBagEx::DropRefugeDarkElfItem(int aIndex, BYTE btMapNumber, BYTE cX, BYTE cY)
{
	if ( this->m_bLoad == FALSE )
		return FALSE;
	...
	if ( this->GetBagCount() > 0 )
	{
		...
		if ( iDropItemRate < g_iRefugeMonsterDarkElfDropZenRate ) 
		{
			MapC[lpObj->MapNumber].MoneyItemDrop(g_iRefugeMonsterDarkElfDropZen, cX, cY);

			LogAddTD("[ Balgass Refuge ][Reward] Dark Elf Item Drop [%s][%s] [%d Zen]",
				lpObj->AccountID, lpObj->Name, g_iRefugeMonsterDarkElfDropZen);
		}
		else
		{
			iDropItemNum = rand()% this->GetBagCount();
			...
			LogAddTD("[ Balgass Refuge ][Reward] Dark Elf Item Drop [%s][%s] : (%d)(%d/%d) Item:(%s)%d Level:%d op1:%d op2:%d op3:%d ExOp:%d",
				lpObj->AccountID, lpObj->Name, btMapNumber, X, Y, ItemAttribute[iType].Name,
				iType, iLevel, iOption1, iOption2, iOption3, iExOption);
