...
bool MWidget::Event(MEvent* pEvent)
{
	...
	// 좌클릭시 이벤트가 전달되어온 계층을 확인할 수 있는 로그
	/*if(pEvent->nMessage==MWM_LBUTTONDOWN)
	{
		MWidget* p = this;
		while (0!=stricmp(p->m_szIDLName, "Shop")) // 상위 부모 이름..
		{
...
// 해상도 변경에 따라서 위젯의 크기를 단순 배율시킬 때 사용한다, 폰트 크기까지 같이 커지는 경우에 사용한다.
// 폰트 크기는 그대로이고 순수하게 프레임창 크기만 변경할 때는 Resize, SetSize 함수군을 사용해야 한다.
// 이 함수는 Resize, SetSize 함수군과는 다르게 anchor를 무시한다. (즉 자식위젯들에도 함께 단순 배율 적용함)
// byIDLWidth, byIDLHeight : IDL 정의가 800*600 기준이므로 현재 해상도가 800*600에 대해서 몇배율인가를 지정.
// byCurrWidth, byCurrHeight : IDL 정의가 없는 위젯은 구해상도->신해상도의 배율로 크기를 조정한다.
void MWidget::MultiplySize(float byIDLWidth, float byIDLHeight, float byCurrWidth, float byCurrHeight)
{
	// idl에서 읽어서 적절한 초기값을 가지고 있는 위젯이라면 그값으로 새 크기를 구한다
	...
	{
		// idl 정의가 없는 위젯은 예전 해상도 -> 새 해상도 간의 배율로 크기를 조정한다
