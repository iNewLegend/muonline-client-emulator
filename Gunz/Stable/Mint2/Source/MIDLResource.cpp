...
MAlignmentMode MIDLResource::GetAlignmentMode(MXmlElement& element)
{
	...
	if(element.GetChildContents(szValue, "HALIGN")==true){
		if(stricmp(szValue, "LEFT")==0) am |= MAM_LEFT;
		else if(stricmp(szValue, "CENTER")==0) am |= MAM_HCENTER;
		else if(stricmp(szValue, "RIGHT")==0) am |= MAM_RIGHT;
	}

	if(element.GetChildContents(szValue, "VALIGN")==true){
		if(stricmp(szValue, "TOP")==0) am |= MAM_TOP;
		else if(stricmp(szValue, "CENTER")==0) am |= MAM_VCENTER;
		else if(stricmp(szValue, "BOTTOM")==0) am |= MAM_BOTTOM;
	}
...
MEdit* MIDLResource::GetEdit(MXmlElement& element)
{
	...
	for (int i = 0; i < iCount; i++)
	{
		...
		else if (!strcmp(szBuf, "NUMBERONLY"))
		{
...
//	char szFontName[4096];
	...
	for (int i = 0; i < iCount; i++)
	{
		...
		if (!stricmp(szBuf, "BORDERSTYLE"))
		{
			...
			if (!stricmp(szBorderStyle, "none")) pPanel->SetBorderStyle(MBS_NONE);
			else if (!stricmp(szBorderStyle, "single")) pPanel->SetBorderStyle(MBS_SINGLE);
		}
...
void MIDLResource::Parse(MXmlElement& element)
{
	...
	else if (!stricmp(szTagName, "COMBOLISTBOXLOOKTEMPLATE"))
	{
	...
	else if (!stricmp(szTagName, "THUMBLOOKTEMPLATE"))
	{
	...
	else if( !stricmp(szTagName, "SLIDERTHUMBLOOKTEMPLATE" ))
	{
	...
	else if (!stricmp(szTagName, "BMBUTTON"))
	{
		GetBmButton(element);
	}
	...
	else if (!stricmp(szTagName, "COMBOBOX"))
	{
		GetComboBox(element);
	}
