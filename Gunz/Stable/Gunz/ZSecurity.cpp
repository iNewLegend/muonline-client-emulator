...
/*	MMatchItemDescMgr* pItemMgr = MMatchItemDescMgr::GetInstance();
	for (MMatchItemDescMgr::iterator i=pItemMgr->begin(); i!=pItemMgr->end(); i++) {
		...
		pDataChecker->AddCheck((BYTE*)&pItem->m_nMaxBullet, sizeof(int), MT_MEMORYFUGITIVE_INT);
		pDataChecker->AddCheck((BYTE*)&pItem->m_nReloadTime, sizeof(int), MT_MEMORYFUGITIVE_INT);
		pDataChecker->AddCheck((BYTE*)&pItem->m_nHP, sizeof(int), MT_MEMORYFUGITIVE_INT);
		pDataChecker->AddCheck((BYTE*)&pItem->m_nAP, sizeof(int), MT_MEMORYFUGITIVE_INT);
	}
	*/
}
...
void ZSetupDataChecker_Game(MDataChecker* pDataChecker)
{
	...
	//아래의 체크섬 검사는 제거하고 대신 crc32검사로 대체하였음 -kimyhwan 
	//// 캐릭터 HP,AP 정보 
/*	ZModule_HPAP* pCharHPAP= (ZModule_HPAP*)ZGetGame()->m_pMyCharacter->GetModule(ZMID_HPAP);
...
*/	/*
	ZCharacterStatus* pCharStatus = ZGetGame()->m_pMyCharacter->GetCharacterStatus();
...
/*	
	//// 캐릭터 장비 아이템 잔탄량 정보
	ZCharacterItem* pCharItem = ZGetGame()->m_pMyCharacter->GetItems();
