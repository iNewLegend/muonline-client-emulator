...
//--------------------------------------------------------------------------------------
extern HRESULT DXUTGetD3DEnumeration( CD3DEnumeration** ppD3DEnum );
...
// Global state
//--------------------------------------------------------------------------------------
CD3DSettingsDlg* g_pD3DSettings = NULL;
...
HRESULT CD3DSettingsDlg::OnCreateDevice( IDirect3DDevice9* pd3dDevice )
{
    ...
    m_Dialog.SetCallback( StaticOnEvent );
...
HRESULT CD3DSettingsDlg::Refresh()
{
    ...
    RECT rcClient = DXUTGetWindowClientRect();
    m_dwWindowWidth  = rcClient.right - rcClient.left;
    m_dwWindowHeight = rcClient.bottom - rcClient.top;
...
//--------------------------------------------------------------------------------------
LRESULT CD3DSettingsDlg::HandleMessages( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
...
{
    if( g_pD3DSettings )
        g_pD3DSettings->OnEvent( nEvent, nControlID, pControl );
}
    ...
    switch( nControlID )
    {
        ...
        case DXUTSETTINGSDLG_OK:
        {
            if( g_DeviceSettings.pp.Windowed )
            {
                ...
                g_DeviceSettings.pp.BackBufferWidth = m_dwWindowWidth;
                g_DeviceSettings.pp.BackBufferHeight = m_dwWindowHeight;
            }
...
void CD3DSettingsDlg::AddResolution( DWORD dwWidth, DWORD dwHeight )
{
    ...
    _snwprintf( strResolution, 50, L"%d by %d", dwWidth, dwHeight );
    strResolution[49] = 0;
...
void CD3DSettingsDlg::AddRefreshRate( DWORD dwRate )
{
    ...
    if( dwRate == 0 )
        wcscpy( strRefreshRate, L"Default Rate" );
    else
        _snwprintf( strRefreshRate, 50, L"%d Hz", dwRate );
...
void CD3DSettingsDlg::AddMultisampleQuality( DWORD dwQuality )
{
    ...
    WCHAR strQuality[50];
    _snwprintf( strQuality, 50, L"%d", dwQuality );
    strQuality[49] = 0;
