...
//	BipIface->RemoveNonUniformScale(1);
    ...
    if (phyExport) {
		...
		if (mcExport) {
			...
			mesh_node->m_physique = new PHYSIQUE [p_num];
			memset(mesh_node->m_physique,0,sizeof(PHYSIQUE)*p_num);
...
//	PatchObject* patch = GetPatchObjectFromNode(node,t,needDel);
	...
	mesh_node->m_face_list = new t_face_info_s[face_num];

	memset(mesh_node->m_point_list,0,sizeof(D3DXVECTOR3)*point_num);
	memset(mesh_node->m_face_list,0,sizeof(t_face_info_s)*face_num);
...
//	mesh_node->m_point_color_num = numCVx;
	...
	if(face_cnt) {
		mesh_node->m_face_normal_list = new t_face_normal_s[face_cnt];
	}

	t_face_normal_s* pTN = NULL;
