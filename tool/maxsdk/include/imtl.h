...
class IMtlParams : public InterfaceServer {
	 ...
	 \par Parameters:
	 <b>TimeChangeCallback *tc</b>\n\n
	 The calllback called when the time changes. See
	 Class TimeChangeCallback. */
...
class LightDesc : public RenderData {
	 public:
			...
			\endcode 
			 ...
			 This is a hint to the material that the light dd not calculate its
...
\par Description:
This class is available in release 2.0 and later only.\n\n
It provides a list of ray / triangle intersection sturctures (<b>struct
...
texmaps, etc. to retireve information about the global rendering enviornment.
...
public:
	 ...
	 */
	 ...
	 The shade context.\n\n
	 <b>Ray \&r</b>\n\n
	 Defines the direction of view of the enviornment. See
	 Class Ray.\n\n
	 <b>BOOL applyAtmos</b>\n\n
	 ...
	 instersected with a transparent object and you want to find out where the
...
Note that it is concievable that more than one raytrace material can be in
...
public:
	...
	*/
	...
	a ray is propogated it is updated for each new surface that is encountered.
	\par Default Implementation:
	...
	*/
	...
	/*! \remarks	 Returns the color of the global enviornment map from the given view
	direction.
	\par Parameters:
	<b>Point3 dir</b>\n\n
	...
	within the same material, say on the diffuse channel and on the shinniness
	...
	*/
	...
	/*! \remarks	 Puts a floatint point value to the cache. See the method <b>GetCache(Texmap
	*map, const AColor \&c)</b> above for detais.
	\par Parameters:
	<b>Texmap *map</b>\n\n
	Points to the texmap storing the cache (usually the plug-ins <b>this</b>
	pointer).\n\n
...
#define MTL_MEDIT_BACKGROUND    (1<<8)  //!< Material has the background shown in Materal Editor.
...
public:
	...
	for it. This method is called on a material/texmap coming into an exsting
...
sub-map of <b>theMtl</b>. The message doesn't propogate up -- it goes directly
...
material has some complicated internal hierachical structure of references, to
	 ...
	 public:
		...
		CoreExport ~MtlBase();
			...
			*/
			...
			propogate the SetThing() to its registered secondary ParamDlgs. In the
			...
			standard SetThing() propogation for that dialog.\n\n
			The Render Effects dialog has a similar arangment to the Materials Editor
			...
			/*! \remarks						This method is called to initalize the interactive renderer Material
			...
			without recomputing.
			\param t The time at which to evaulate the material.
...
class Texmap: public MtlBase {
	 public:
			...
			result in RGB. <b>forceW</b> and <b>forceH</b>, if non-zero, overide
			...
			level of the imbedded Texout object, principally by importing plug-ins.
...
***********/
			...
			/*! \remarks			Returns the amound of displacement along the normal of the surface at the
...
is used by the Straus shader for example. If it sees a reflection map present
...
class MtlBaseList: public Tab<MtlBaseHandle> {
	 public:
			/*! \remarks Adds the specified MtlBase to the list.
			\par Parameters:
			<b>MtlBase *m</b>\n\n
			The MtlBase to add.\n\n
			<b>BOOL checkUnique=TRUE</b>\n\n
			If TRUE this method checks to make sure the MtlBase is unique, and will
			only add it if so.
			\return  Nonzero if the MtlBase was added; otherwise zero. */
			CoreExport int AddMtl(MtlBase *m, BOOL checkUnique=TRUE);
			/*! \remarks Finds the specified MtlBase in this material list and
			returns its index. Returns -1 if not found.
			\par Parameters:
			<b>MtlBase *m</b>\n\n
			The MtlBase to find. */
			CoreExport int FindMtl(MtlBase *m);
			/*! \remarks Finds the specified material by name and returns its index.
			Returns -1 if not found.
			\par Parameters:
			<b>MSTR\& name</b>\n\n
			The name to find. */
			CoreExport int FindMtlByName(MSTR& name);
			/*! \remarks Removes the specified MtlBase from the list.
			\par Parameters:
			<b>int n</b>\n\n
			The index of the MtlBase to remove. */
			void RemoveEntry(int n) { Delete(n,1); }
			/*! \remarks Removes all MtlBases from the list. */
			void Empty() { Resize(0); }
	 };
...
For additional information on this class see the Advanced Topics sectoin
...
class DADBitmapCarrier: public ReferenceTarget {
	 ...
	 RefTargetHandle Clone(RemapDir &remap = DefaultRemapDir()) { assert(0); return NULL; }
