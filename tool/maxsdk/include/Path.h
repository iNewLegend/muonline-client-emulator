...
#ifndef _FILE_IO_PATH_H_
#define _FILE_IO_PATH_H_
...
public:
	 ...
	 *     a path object.
	 */
	UtilExport Path(const TSTR &pathString);

	/*!
	 * \brief Construct from a null-terminated path-filename TCHAR array.
	 *
	 * \param pathString path & filename (must be null-terminated).
	 * \pre pathString is null terminated.
	 */
	UtilExport Path(const TCHAR *pathString);
	 ...
	 */
	UtilExport Path& SetPath(const TCHAR *pathString);

	/*!
	 * \brief Set this Path from a TSTR.
	 *
	 * Equivalent to the assignment operator, but more verbose.
	 * \param pathString TSTR to copy into this path.
	 */
	UtilExport Path& SetPath(const TSTR &pathString);
	 ...
	 * \param pathString null-terminated character string to copy into this path.
	 */
	UtilExport Path& operator=(const TCHAR *pathString);
	 ...
	 * \param pathString TSTR to copy into this path.
	 */
	UtilExport Path& operator=(const TSTR &pathString);
    ...
    * e.g. "\\abc\\def" is rooted at backslash.
	 ...
	 */
	UtilExport TSTR GetString() const;
	 ...
	 * this object.
	 */
	UtilExport const TCHAR* GetCStr() const;
