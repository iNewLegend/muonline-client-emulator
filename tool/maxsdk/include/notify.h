...
//! \brief Sent before a file is saved ( callParam is TCHAR * to file name )
#define NOTIFY_FILE_PRE_SAVE			0x0000000D 
//! \brief Sent after a file is saved ( callParam is TCHAR * to file name )
...
/** The \ref NotifyInfo structure pointer callParam is passed the TCHAR * to the 
...
/** Call Param is a pointer to <b>struct{ TCHAR* oldname; TCHAR* newname; }</b> \n
...
//! \brief Sent when the system is updating it's custom colors.
/** If a plug-in has created a toolbar with any 
<b>MaxBmpFileIcons</b> on it, it should register this callback for color changing, and call 
<b>ICustToolbar::ResetIconImages()</b> on the toolbar. See Classes ICustToolbar and MAXBmpFileIcon. */
...
//! \brief Sent on scene undo (callParam is TCHAR* with the undo entry name)
#define NOTIFY_SCENE_UNDO                   0x00000061 
//! \brief Sent on scene redo (callParam is TCHAR* with the redo entry name)
...
//! \brief Sent after an XRef scene is successfully merged
...
/** callParam is pointer to struct{ TCHAR* oldname; TCHAR* newname; INode* node} */
...
All the SceneState notifications have CallParam as a TCHAR* with the SceneState name except 
...
/** CallParam is pointer to struct{ const TCHAR* oldname; const TCHAR* newname; } */
...
All contain a CallParam that points to a struct{ int iProcessType; const TCHAR* filename; }.
...
// Added LAM 15-Mar-05
//! \brief Provides a notification that a ClassDesc was loaded from a plugin dll.
/** callParam is pointer to the ClassDesc. */
...
situations The callParam is a struct{ TCHAR * fileName, TCHAR* paramName, Mtl* dxMaterial, bool forceUpdate} 
