...
#ifndef __IBRUSHPRESETS__H
#define __IBRUSHPRESETS__H
#include "maxheap.h"
...
class IBrushPresetContext: public MaxHeapOperators {
	public:
		...
		virtual TCHAR*		GetContextName() = 0;
		...
		/*! \param[in] paramID The ID of the parameter value */
		virtual TCHAR*		GetParamName( int paramID ) = 0;
		...
		virtual TCHAR*		GetDisplayParamName( int paramID )		{return GetParamName(paramID);}
		...
		virtual BOOL		IsDisplayParam( int paramID )			{return TRUE;}
		...
		virtual BOOL		IsTransientParam( int paramID )			{return FALSE;}
