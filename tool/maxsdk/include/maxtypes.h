...
//@{
typedef enum {
	IO_OK=0,	//!< The result was acceptable - no errors.
	/*! Indicates the end of the chunks at a certain level have been reached. It is used as a
	signal to terminates the processing of chunks at that level. */
	IO_END=1,	//!< Returned from ILoad::OpenChunk().
	/*! Note that the plug-in should not put up a message box if a read error occurred. It should
	simply return the error status. This prevents a excess of messages from appearing. */
	IO_ERROR=2	//!< Returned in an error occurred.
} IOResult; 
//@}
...
typedef enum 
{
	//! \brief File IO concerns a scene file (.max, .viz). See \ref NOTIFY_FILE_PRE_OPEN, \ref NOTIFY_FILE_POST_OPEN
	IOTYPE_MAX = 0, 
	//! \brief File IO concerns a material library file (.mat). See \ref NOTIFY_FILE_PRE_OPEN, \ref NOTIFY_FILE_POST_OPEN
	IOTYPE_MATLIB = 1, 
	//! \brief File IO concerns a render preset file (.rps). See \ref NOTIFY_FILE_PRE_OPEN, \ref NOTIFY_FILE_POST_OPEN
