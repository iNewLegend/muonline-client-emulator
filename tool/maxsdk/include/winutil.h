...
public:
	...
	MaxLocaleHandler(int category = LC_ALL, const MCHAR* localeSetting = _M("C"));
...
inline MaxLocaleHandler::MaxLocaleHandler(int category, const MCHAR* localeSetting) 
: m_oldLocale(setlocale(category, NULL)),		// Save old locale
  m_category(category)
{
	// Set new locale
	setlocale(category, localeSetting);
}
...
inline MaxLocaleHandler::~MaxLocaleHandler() {
   ...
   {
      // Restore old locale
      setlocale(m_category, m_oldLocale.data());
   }
}
...
class ToolTipExtender : public MaxHeapOperators
{
	public:
		...
		CoreExport virtual void SetToolTip( HWND hWnd, const TSTR& toolTipIn );
		...
		CoreExport virtual void GetToolTip( HWND hWnd, TSTR& toolTipOut );
...
\code
	case WM_CHAR:
		return MaxSDK::SearchComboBox( hWnd, (TCHAR)wParam, _T(" "));
\endcode
...
*/
CoreExport bool SearchComboBox( HWND hWnd, TCHAR key, const TCHAR* szTrim = NULL );
...
\code
	case WM_CHAR:
		return MaxSDK::SearchListBox( hWnd, (TCHAR)wParam, _T(" "));
\endcode
...
\return true if match found, false if not
*/
CoreExport bool SearchListBox( HWND hWnd, TCHAR key, const TCHAR* szTrim = NULL );
