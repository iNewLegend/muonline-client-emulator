...
#ifndef __BIPED8API__
#define __BIPED8API__
...
class IBipedCopy: public MaxHeapOperators
{
  public:
	...
	virtual bool SetName(TCHAR *name) = 0;
...
class ICopyCollection: public MaxHeapOperators
{
  public:
    ...
    virtual IBipedCopy *GetCopy(int copyType, TCHAR *name, int *index) = 0;
...
//@{
		...
		//! \return true if the function was successful, false if not
		virtual bool SetTwistPoseName(int keytrack, int index, TCHAR *name)=0;
		...
		virtual TCHAR *GetTwistPoseName(int keytrack, int index)=0;
		...
		//! \return true if the function was successful, false if not
		virtual int		LoadFigNoTwists		(TCHAR *fname, BOOL redraw = false, BOOL msgs = false)=0; 
		...
		//! \param[in] msgs - pop up error dialogs if an error occurs
		//! \return true if the function was successful, false if not
		virtual int		LoadFigJustTwists	(TCHAR *fname, BOOL redraw = false, BOOL msgs = false)=0; 
//@}
...
//@{
		...
		//! \return the specified copy collection, NULL if the index was out of bounds
		virtual ICopyCollection *GetCopyCollection(TCHAR *name, int *index) const = 0;

		//! \brief Creates a copy collection.
		//! \param[in] colname - name of new copy collection
		//! \return the newly created copy collection
		virtual	ICopyCollection *CreateCopyCollection(TCHAR *colname) = 0;
