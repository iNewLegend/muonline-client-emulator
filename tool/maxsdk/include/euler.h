...
DllExport void QuatToEuler(const Quat &q, float *ang, int type, bool flag = false);	// flag added 001101  --prs.
DllExport void EulerToQuat(float *ang, Quat &q,int type);
DllExport void MatrixToEuler(const Matrix3 &mat, float *ang, int type, bool flag = FALSE);
DllExport void EulerToMatrix(float *ang, Matrix3 &mat, int type);
DllExport float GetEulerQuatAngleRatio(Quat &quat1,Quat &quat2, float *euler1, float *euler2, int type = EULERTYPE_XYZ);
DllExport float GetEulerMatAngleRatio(Matrix3 &mat1,Matrix3 &mat2, float *euler1, float *euler2, int type = EULERTYPE_XYZ);
...
protected:
	DllExport Point3	ToEulerAngles(EulerType et) const;
	DllExport Quat	ToQuat() const;
