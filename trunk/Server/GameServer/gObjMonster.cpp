...
void gObjMonsterProcess(LPOBJ lpObj)
{
	...
	if ( lpObj->m_ActState.Attack == 1 )
	{
		if ( TMonsterSkillManager::CheckMonsterSkill(lpObj->Class) )
		{
			...
			if ( rand()%4 == 0 )
			{
				PMSG_ATTACK pAttackMsg(120,lpObj->TargetNumber,lpObj->Dir);
...
void gObjMonsterMagicAttack(LPOBJ lpObj, int iMonsterClass)
{
	...
	pCount.h.c = 0xC1;
	pCount.h.headcode = GMSG_BEATTACK;
	pCount.h.size = 0;
...
void gObjUseMonsterSpecialAbillity(LPOBJ lpMonsterObj)
{
	...
	{
		PMSG_MAGICATTACK pAttackMsg(1,lpMonsterObj->TargetNumber,0);
...
void gObjMonsterBeattackRecv(BYTE * lpRecv, int aIndex)
{
	...
	int tNumber;
	CMagicInf * lpMagic = gObjGetMagic(&gObj[aIndex], lpCount->GetMagicNumber());
...
void gObjMonsterAttack(LPOBJ lpObj, LPOBJ lpTargetObj)
{
		...
		lpObj->Class == 260 || lpObj->Class == 268 )
	{
		if ( rand()%2 )
		{
			PMSG_MAGICATTACK pAttackMsg(0,lpObj->TargetNumber,0);
	...
	{
		PMSG_MAGICATTACK pAttackMsg(rand()%6+1,lpObj->TargetNumber,0);
		CGMagicAttack(&pAttackMsg, lpObj->m_Index);

		if ( pAttackMsg.GetMagicNumber() == 1 || pAttackMsg.GetMagicNumber() == 2 || pAttackMsg.GetMagicNumber() == 0 )
		{
	...
	{
		PMSG_MAGICATTACK pAttackMsg(rand()%2+1,lpObj->TargetNumber,0);
		CGMagicAttack(&pAttackMsg, lpObj->m_Index);
		return;
	}
          ...
          || lpObj->Class == 274 )
	{
		if ( (rand()%2) )
		{
			PMSG_MAGICATTACK pAttackMsg(1,lpObj->TargetNumber,0);
			CGMagicAttack(&pAttackMsg, lpObj->m_Index);
			return;
		}
		AttackType = 0;
...
void gObjMonsterDieGiveItem(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	if ( g_QuestInfo.MonsterItemDrop(lpObj) ) //Season3 Quest Monster Fix
	return;
	...
	if ( lpObj->Class == 340 )	// Dark Elf
	{
		...
		if ( lpObj->Class == 42 )	// Big Dragon
	{
		if ( g_bBigDragonItemDrop )
		{
			int ItemDropRate = rand()%10000;

			if ( ItemDropRate <= g_iBigDragonItemDropRate )
			{
				int iMaxHitUser = gObjMonsterTopHitDamageUser(lpObj);

				LogAddTD("[ BigDragon ][Reward] Monster BigDragon ItemDrop MaxHitUser [%s][%s]",
					lpTargetObj->AccountID, lpTargetObj->Name);
				...
				BigDragonItemBagOpen(lpTargetObj, lpObj->MapNumber, cDropX, cDropY);
			}
		}
		return;
	}

		if ( lpObj->Class == 59 )	// Zaikan
	{
		if ( g_bZaikanItemDrop )
		{
			int ItemDropRate = rand()%10000;

			if ( ItemDropRate <= g_iZaikanItemDropRate )
			{
				int iMaxHitUser = gObjMonsterTopHitDamageUser(lpObj);

				LogAddTD("[ Zaikan ][Reward] Monster Zaikan ItemDrop MaxHitUser [%s][%s]",
					lpTargetObj->AccountID, lpTargetObj->Name);
				...
				ZaikanItemBagOpen(lpTargetObj, lpObj->MapNumber, cDropX, cDropY);
			}
		}
		return;
	}

		if ( lpObj->Class == 291 )	// Boss
	{
		if ( g_bBigBaliItemDrop )
		{
			int ItemDropRate = rand()%10000;

			if ( ItemDropRate <= g_iBigBaliItemDropRate )
			{
				int iMaxHitUser = gObjMonsterTopHitDamageUser(lpObj);

				LogAddTD("[ BOSS ][Reward] Monster Boss ItemDrop MaxHitUser [%s][%s]",
					lpTargetObj->AccountID, lpTargetObj->Name);
				...
				BigBaliItemBagOpen(lpTargetObj, lpObj->MapNumber, cDropX, cDropY);
			}
		}
		return;
	}
	...
	//Need Finish Decompilation leo123
	#if(GS_CASTLE==1)
	...
	if ( lpObj->Class == 295 )
	{
		gObjMonsterTopHitDamageUser(lpObj);
	...
	ExtDropPer = rand()%200000;	// Excellent Drop Percent #warning
	...
	if ( ExtDropPer == TRUE )
	{
		DropItem = g_MonsterItemMng.GetItem(lpObj->Level-30);
...
BOOL gEventMonsterItemDrop(LPOBJ lpObj, LPOBJ lpTargetObj)
{
		...
		if ( lpObj->Level >= gSetItemInCastleHuntZoneDropLevel ) //Good
		{
			if ( (rand()%10000) < gSetItemInCastleHuntZoneDropRate )
			{
				...
				if ( OBJMAX_RANGE(MaxHitUser) )
				{
					...
					MakeRewardSetItem(lpObj->m_Index, lpObj->X, lpObj->Y, 0 , lpObj->MapNumber);
	...
	{
		if ( (rand()%10000) < g_ServerRates.g_CondorFlameDropRate )
		{
		...
		if ( (rand()%10000) < g_ServerRates.g_CondorFeatherDropRate )
		{
			type = ItemGetNumberMake(13,53);	// Condor FeatherDrop
			level = 0;
			x = lpObj->X;
			y = lpObj->Y;
			int MaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
...
void CQeustNpcTeleport::Run(int aIndex)
{
	...
	{
		PMSG_MAGICATTACK_RESULT pMsg(6,aIndex,aIndex);
