...
BOOL CObjUseSkill::EnableSkill(WORD Skill)
{
	if(Skill != AT_SKILL_STORM
		...
		&& Skill != AT_SKILL_MAGICSPEED_UP
		&& Skill != AT_SKILL_MAGICDEFENSE_UP
...
void CObjUseSkill::UseSkill(int aIndex, CMagicInf * lpMagic, BYTE x, BYTE y, BYTE dir, BYTE TargetPos, int aTargetIndex)
{
	...
	if(usemana >= 0)
	{
		...
		{
			LogAddTD("[%] ???? ??????? ????",gObj[aIndex].AccountID);
			return;
		}
		...
		if(usebp >= 0)
		{
			...
			PMSG_DURATION_MAGIC_SEND pMsg(lpMagic->m_Skill,aIndex,x,y,dir);
...
//////////////////////////////////////////////////////////////////////////////////////////////
		...
		{ // Summoner
			this->SkillBookNumber12(aIndex,aTargetIndex,lpMagic);
		}
		else if(lpMagic->m_Skill == AT_SKILL_MAGICSPEED_UP )
		{ // Summoner
			this->SkillMagicSpeedUp(aIndex,0);
		}
		else if(lpMagic->m_Skill == AT_SKILL_MAGICDEFENSE_UP )
		{ // Summoner
			this->SkillMagicDefenseUp(aIndex,0);
		}
		else if(lpMagic->m_Skill == AT_SKILL_GHOST_PHANTOM )
		{ // Summoner
			this->SkillGhostPhantomAttack(aIndex,aTargetIndex,lpMagic);
		}
		else if(lpMagic->m_Skill == AT_SKILL_RED_STORM)
		{ // Summoner
			this->SkillRedStorm(aIndex,aTargetIndex,lpMagic);
		}
//////////////////////////////////////////////////////////////////////////
		else if(lpMagic->m_Skill == AT_SKILL_EXPLOTION )
		{
			this->SkillExplotion(aIndex,aTargetIndex,lpMagic,x,y,bCombo);
		}
		else if(lpMagic->m_Skill == AT_SKILL_SWORD_SLASH )
		{
			this->SkillSwordSlash(aIndex, aTargetIndex, lpMagic, TargetPos);
		}
		else if(lpMagic->m_Skill == AT_SKILL_LIGHTING_STORM )
		{
			this->SkillLightingStorm(aIndex,aTargetIndex,lpMagic);
		}
		...
		{
			this->SkillBirdsAttack(aIndex,aTargetIndex,lpMagic,TargetPos);
		}
	}
}
...
BOOL CObjUseSkill::RunningSkill(int aIndex, int aTargetIndex, CMagicInf * lpMagic, BOOL bCombo)
{
	...
	case AT_SKILL_CALLMON1:	return this->SkillMonsterCall(lpObj->m_Index,37,lpObj->X-1,lpObj->Y+1); // Goblin
	case AT_SKILL_CALLMON2:	return this->SkillMonsterCall(lpObj->m_Index,305,lpObj->X-1,lpObj->Y+1); // Stone Golem
	case AT_SKILL_CALLMON3:	return this->SkillMonsterCall(lpObj->m_Index,61,lpObj->X-1,lpObj->Y+1); // Assassin
	case AT_SKILL_CALLMON4:	return this->SkillMonsterCall(lpObj->m_Index,74,lpObj->X-1,lpObj->Y+1); // Yeti
	case AT_SKILL_CALLMON5:	return this->SkillMonsterCall(lpObj->m_Index,360,lpObj->X-1,lpObj->Y+1); // Dark Knight
	case AT_SKILL_CALLMON6:	return this->SkillMonsterCall(lpObj->m_Index,150,lpObj->X-1,lpObj->Y+1); // Bali
	case AT_SKILL_CALLMON7:	return this->SkillMonsterCall(lpObj->m_Index,151,lpObj->X-1,lpObj->Y+1); // Soldier
...
void CObjUseSkill::MaGumSkillDefenseDown(int aIndex, int aTargetIndex, int skill_level)
{
	...
	LogAdd("?集?? ?????????????? : %d",10); // !!! Need Translation
...
void CObjUseSkill::KnightSkillAddLife(int aIndex, int skill_level)
{
	...
	// LEO JOB
	// SWELLING LIFE EHP
	int iaddLifepower = 12 + (lpObj->Vitality + lpObj->AddVitality) / /*100*/ g_ServerValues.g_DkAddLifeSkillVitPercent + (lpObj->Energy + lpObj->AddEnergy) / g_ServerValues.g_DkAddLifeSkillEnePercent /*20*/ + viewpercent;
	//iaddLifepower = iaddLifepower * 3;
	int iLifeTime = g_ServerValues.g_DkAddLifeSkillDuration + (lpObj->Energy + lpObj->AddEnergy) / 10;/// //*10*/30;
	int iaddlife;
	...
	if(partynum == -1)
	{
		...
		lpObj->m_ViewSkillState[eVS_SWELL_LIFE] = 1;
	...
	else
	{
		for(int n = 0; n < MAX_USER_IN_PARTY; n++)
		{
			if(ApplyPartyIndex[n]!= -1)
			{
				...
				lpPartyObj->m_ViewSkillState[eVS_SWELL_LIFE] = 1;
...
void CObjUseSkill::WizardMagicDefense(int aIndex, int aTargetIndex, int skill_level)
{
	...
	lpTargetObj->m_WizardSkillDefense = 10 + (lpObj->Dexterity + lpObj->AddDexterity) / /*50*/655 + (lpObj->Energy + lpObj->AddEnergy) / /*200*/820;
	lpTargetObj->m_WizardSkillDefenseTime = 60 + (lpObj->Energy + lpObj->AddEnergy) / /*40*/110;
...
void CObjUseSkill::SkillDefense(int aIndex, int aTargetIndex, int skill_level)
{
	...
	lpTargetObj->m_SkillDefenseTime = 70;
...
void CObjUseSkill::SkillAttack(int aIndex, int aTargetIndex, int skill_level)
{
	...
	lpTargetObj->m_SkillAttackTime = 70;
...
BOOL CObjUseSkill::SkillChangeUse(int aIndex)
{
	...
	{
		// huihui GCStateInfoSend(lpObj, 0, 0x8000000);
	}
...
BOOL CObjUseSkill::SkillHellFire2(int aIndex, int aTargetIndex, CMagicInf * lpMagic)
{
	...
	{
		LogAddTD("[%s][%s] ????????2 ?????風?????紫 ??????? ???? ?????",lpObj->AccountID,lpObj->Name);
		return false;
	}
...
BOOL CObjUseSkill::SkillHellFire2Start(int aIndex, CMagicInf * lpMagic)
{
	...
	if(lpObj->SkillHellFire2State != 0)
	{
		LogAddTD("[%s][%s] ????????2 ?????風?????? ? ?風???????? ????",lpObj->AccountID,lpObj->Name);
		return false;
	}
			...
			if(OBJMAX_RANGE(tObjNum))
			{
				if(lpObj->VpPlayer2[count].type == OBJ_USER)
				{
					if(gObjCalDistance(lpObj,&gObj[tObjNum]) < 5)
					{
						...
						gObj[tObjNum].m_ViewSkillState[eVS_SACRIFICE] = 1;
				...
				if(OBJMAX_RANGE(tObjNum))
				{
					if(gObjCalDistance(lpObj,&gObj[tObjNum]) <= 4)
					{
						if(SkillElectricSparkHitBox.HitCheck(iangle,lpObj->X,lpObj->Y,gObj[tObjNum].X,gObj[tObjNum].Y))
						{
							...
							gObj[tObjNum].m_ViewSkillState[eVS_STUN] = 1;
			...
			if(OBJMAX_RANGE(tObjNum) && lpObj->VpPlayer2[count].type == OBJ_USER)
			{
				if(gObjCalDistance(lpObj,&gObj[tObjNum]) < 5)
				{
					if(lpObj->GuildNumber > 0 && gObj[tObjNum].GuildNumber > 0)
					{
						...
						gObj[tObjNum].m_ViewSkillState[eVS_SWELL_MANA] = 1;
...
BOOL CObjUseSkill::SkillCloaking(int aIndex, int aTargetIndex, CMagicInf * lpMagic)
{
	...
	lpTargetObj->m_ViewSkillState[eVS_TRANSPARENCY] = 1;
		...
		if(lpObj->VpPlayer2[count].state != 0)
		{
			...
			if(OBJMAX_RANGE(tObjNum) && lpObj->VpPlayer2[count].type == OBJ_USER)
			{
				if(gObjCalDistance(lpObj,&gObj[tObjNum]) < 5)
				{
					...
					gObj[tObjNum].m_ViewSkillState[eVS_TRANSPARENCY] = 0;
...
BOOL CObjUseSkill::RemoveCloakingEffect(int aIndex)
{
	...
	gObj[aIndex].m_ViewSkillState[eVS_TRANSPARENCY] = 0;
...
int CObjUseSkill::SkillInfinityArrow(int aIndex,int aTargetIndex,CMagicInf * lpMagic)
{
	...
	lpObj->m_ViewSkillState[eVS_INFINITY_ARROW] = 1;
...
int CObjUseSkill::SkillDrainLife(int aIndex,int aTargetIndex,CMagicInf * lpMagic)
{
	...
	int drainLife = (((lpObj->Energy * 100)/32000) * lpObj->MaxLife)/100;
...
void CObjUseSkill::SkillSleep(int aIndex, int aTargetIndex, int skill_level)
{
    int skillSuccess = 1;
	LPOBJ lpObj = &gObj[aIndex];
    LPOBJ lpTargetObj = &gObj[aTargetIndex];
...
void CObjUseSkill::SkillReflex(int aIndex,int skill_level)
{
	...
	if(partynum == -1)
	{
		...
		lpObj->m_SummonerSkillReflex = reflectvalue;
		lpObj->m_SummonerSkillReflexTime = skillTime;
		GCStateInfoSend(lpObj,1,eVS_SKILL_DAMAGEREFLECTION);
		GCMagicAttackNumberSend(lpObj,0xD9,lpObj->m_Index,skillSuccess);
	}
	else
	{
		for(int n=0;n<5;n++)
		{
			if(ApplyPartyIndex[n] != -1)
			{
				...
				lpPartyObj->m_SummonerSkillReflex = reflectvalue;
				lpPartyObj->m_SummonerSkillReflexTime = skillTime;
...
BOOL CObjUseSkill::SkillRedStorm(int aIndex, int aTargetIndex, CMagicInf * lpMagic)
{
	LPOBJ lpObj = &gObj[aIndex];
    ...
    for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		if(this->CheckViewportTarget(lpObj,index,aTargetIndex,lpObj->VpPlayer2[n].type) == 0)
		{
		    continue;
		}

		if(gObjCalDistance(lpObj,&gObj[index]) < 5)
		{
...
bool CObjUseSkill::SkillLightingStorm(int aIndex,int bIndex,CMagicInf* lpMagic) //GOOD 237
{
    LPOBJ lpObj = &gObj[aIndex];
...
bool CObjUseSkill::SkillChainLightning(int aIndex,int bIndex,CMagicInf* lpMagic)
{
    ...
    gObjAttack(lpObj,lpTarget,lpMagic,0,1,0,0);

	return 1;
}

void CObjUseSkill::SkillMagicSpeedUp(int aIndex,int skill_level)
{
    int skillSuccess = 1;
	LPOBJ lpObj = &gObj[aIndex];
	int number;
	...
	int speedUpValue = ((lpObj->Energy + lpObj->AddEnergy)/35+40);
	int skillTime = ((lpObj->Energy + lpObj->AddEnergy)/30+2);
	...
	if(partynum == -1)
	{
		lpObj->m_MagicSpeed += speedUpValue;
		lpObj->m_SummonerSkillSpeedUp = speedUpValue;
		lpObj->m_SummonerSkillSpeedUpTime = skillTime;
		GCStateInfoSend(lpObj,1,eVS_SKILL_NICEWEEK);
		GCMagicAttackNumberSend(lpObj,0xDD,lpObj->m_Index,skillSuccess);
	}
	else
	{
		for(int n=0;n<5;n++)
		{
			...
			{
				lpPartyObj = &gObj[ApplyPartyIndex[n]];
				lpPartyObj->m_SummonerSkillSpeedUp = speedUpValue;
				lpPartyObj->m_SummonerSkillSpeedUpTime = skillTime;
				GCStateInfoSend(lpPartyObj,1,eVS_SKILL_NICEWEEK);
				GCMagicAttackNumberSend(lpObj,0xDD,lpPartyObj->m_Index,skillSuccess);
			}
		}
	}
}

void CObjUseSkill::SkillMagicDefenseUp(int aIndex,int skill_level)
{
    int skillSuccess = 1;
	LPOBJ lpObj = &gObj[aIndex];
	int number;
	...
	int defenseUpValue = ((lpObj->Energy + lpObj->AddEnergy)/50+10);
	int skillTime = ((lpObj->Energy + lpObj->AddEnergy)/15+25);

	if(defenseUpValue > 100)
	{
		defenseUpValue = 100;
	}
	...
	if(partynum == -1)
	{
		lpObj->m_MagicDefense += defenseUpValue;
		lpObj->m_SummonerSkillDefenseUp = defenseUpValue;
		lpObj->m_SummonerSkillDefenseUpTime = skillTime;
		GCStateInfoSend(lpObj,1,eVS_SKILL_INNERBEIYSEYON);
		GCMagicAttackNumberSend(lpObj,0xDE,lpObj->m_Index,skillSuccess);
	}
	else
	{
		for(int n=0;n<5;n++)
		{
			...
			{
				lpPartyObj = &gObj[ApplyPartyIndex[n]];
				lpPartyObj->m_MagicDefense += defenseUpValue;
				lpPartyObj->m_SummonerSkillDefenseUp = defenseUpValue;
				lpPartyObj->m_SummonerSkillDefenseUpTime = skillTime;
				GCStateInfoSend(lpPartyObj,1,eVS_SKILL_INNERBEIYSEYON);
				GCMagicAttackNumberSend(lpObj,0xDE,lpPartyObj->m_Index,skillSuccess);
			}
		}
	}
}
...
void CObjUseSkill::SkillSwordPower(int aIndex, int aTargetIndex, int skill_level)
{
	...
	if(lpObj->Class != 5 || lpTargetObj->Type == OBJ_MONSTER)
	    return;
	...
	int SwordPower = 10 + (lpObj->Dexterity + lpObj->AddDexterity) / /*MSAgilityDivisor[skill_level] +*/ (lpObj->Energy + lpObj->AddEnergy) /*/ MSEnergyDivisor[skill_level]*/;
	int SwordPowerTime = 60 + (lpObj->Energy + lpObj->AddEnergy) /*/ MSEnergyDivisorTime[skill_level]*/;


	if(partynum == -1)
	{
		lpObj->m_SummonerSkillSwordPower = SwordPower;
		lpObj->m_Defense  -= SwordPower/100;
		lpObj->m_SummonerSkillSwordPowerTime = SwordPowerTime;
		lpObj->m_AttackDamageMax += SwordPower;
		lpObj->m_AttackDamageMin += SwordPower;
		lpObj->m_MagicDamageMax += SwordPower;
		lpObj->m_MagicDamageMin += SwordPower;

		GCStateInfoSend(lpObj,1,eVS_SKILL_SWORDPOWER);
		GCMagicAttackNumberSend(lpObj,0xDA,lpObj->m_Index,skillSuccess);
	}
	else
	{
		for(int n=0;n<5;n++)
		{
			...
			{
				lpPartyObj = &gObj[ApplyPartyIndex[n]];
				lpPartyObj->m_SummonerSkillSwordPower = SwordPower;
				lpPartyObj->m_Defense -= SwordPower/100;
				lpPartyObj->m_SummonerSkillSwordPowerTime = SwordPowerTime;
				lpPartyObj->m_AttackDamageMax += SwordPower;
				lpPartyObj->m_AttackDamageMin += SwordPower;
				lpPartyObj->m_MagicDamageMax += SwordPower;
				lpPartyObj->m_MagicDamageMin += SwordPower;

				GCStateInfoSend(lpPartyObj,1,eVS_SKILL_SWORDPOWER);
				GCMagicAttackNumberSend(lpObj,0xDA,lpPartyObj->m_Index,skillSuccess);
			}
		}
	}
}

bool CObjUseSkill::SkillExplotion(int aIndex,int bIndex,CMagicInf *lpMagic,BYTE x,BYTE y,BOOL combo) //GOOD 232
{
    LPOBJ lpObj = &gObj[aIndex];
    ...
    for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		if(this->CalDistance(x,y,gObj[index].X,gObj[index].Y) < 3)
		{
            gObjAddAttackProcMsgSendDelay(lpObj,50,index,400,lpMagic->m_Skill,combo);
