	...
	while ( true )
	{
		...
		while ( true )
		{
			...
			else if ( type == 2 ) // Gifts
			{
				...
				int index = this->m_iCC_GIFTS_COUNT;

				m_Gifts[index].SubType = TokenNumber;

				Token = GetToken();
				m_Gifts[index].Type = TokenNumber;

				Token = GetToken();
				m_Gifts[index].Level = TokenNumber;

				this->m_iCC_GIFTS_COUNT++;
...
void CChaosCastle::CheckSync(int iChaosCastleIndex)
{
	...
	for ( std::vector<CHAOSCASTLE_START_TIME>::iterator it = this->m_vtChaosCastleOpenTime.begin() ; it != this->m_vtChaosCastleOpenTime.end() ; it++ )
	{
		if ( (sysTime.wHour * 60 + sysTime.wMinute) < (it->m_iHour * 60 + it->m_iMinute) )
		{
			if ( ( iMIN_HOUR * 60 + iMIN_MINUTE ) > ( it->m_iHour * 60 + it->m_iMinute ) )
			{
				bTIME_CHANGED = TRUE;
...
void CChaosCastle::PullObjInnerPlace(int iChaosCastleIndex, int iTRAP_STEP)
{
	...
	for ( int iAXIS = 0; iAXIS < MAX_CC_TRAP_STEP ; iAXIS++)
	{
		for ( int iMAPX = g_rtPOINT_TRAP[iAXIS].left ; iMAPX <= g_rtPOINT_TRAP[iAXIS].right ; iMAPX ++ )
		{
			...
			{
				BYTE btMapAttr = MapC[iChaosCastleIndex + MAP_INDEX_CHAOSCASTLE1].GetAttr(iMAPX, iMAPY);
	...
	for ( int i=0;i<MAX_CHAOSCASTLE_USER;i++)
	{
		...
		{
			if ( gObjIsConnected(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex) ==TRUE && gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].MapNumber == (iChaosCastleIndex + MAP_INDEX_CHAOSCASTLE1) )
			{
				...
				BYTE btMapAttr = MapC[iChaosCastleIndex + MAP_INDEX_CHAOSCASTLE1].m_attrbuf[iSY * 256 + iSX] & 0x08;

				if ( btMapAttr == 8 )
				{
					int n=0;
					for ( ;n<MAX_CC_TRAP_STEP;n++)
					{
	...
	{
		if ( MapC[iChaosCastleIndex + MAP_INDEX_CHAOSCASTLE1].m_cItem[iMAP_ITEM].IsItem() )
		{
			int x = MapC[iChaosCastleIndex + MAP_INDEX_CHAOSCASTLE1].m_cItem[iMAP_ITEM].px;
			int y = MapC[iChaosCastleIndex + MAP_INDEX_CHAOSCASTLE1].m_cItem[iMAP_ITEM].py;
			...
			{
				MapC[iChaosCastleIndex + MAP_INDEX_CHAOSCASTLE1].m_cItem[iMAP_ITEM].m_State = 8;
				MapC[iChaosCastleIndex + MAP_INDEX_CHAOSCASTLE1].m_cItem[iMAP_ITEM].Give = true;
				MapC[iChaosCastleIndex + MAP_INDEX_CHAOSCASTLE1].m_cItem[iMAP_ITEM].live = false;
			}
		}
	}
}
...
void CChaosCastle::SetMapAttrHollow(int iChaosCastleIndex, int iTRAP_STEP)
{
	...
	for ( int a=0;a<MAX_CC_HOLLOW_ZONE;a++)
	{
		for ( int b=::g_iChaosCastle_MapHollowZone[iTRAP_STEP][a][0] ; b<=::g_iChaosCastle_MapHollowZone[iTRAP_STEP][a][2] ; b++)
		{
			...
			{
				MapC[iChaosCastleIndex +MAP_INDEX_CHAOSCASTLE1].m_attrbuf[ c * 256 + b] |= 8;
			}
		}
	}
...
void CChaosCastle::SetState_Closed(int iChaosCastleIndex)
{
	...
	{
		if ( gObj[n].MapNumber == (iChaosCastleIndex + MAP_INDEX_CHAOSCASTLE1) )
		{
...
int  CChaosCastle::SetMonster(int iChaosCastleIndex)
{
	...
	for ( int n=0;n<gMSetBase.m_Count;n++)
	{
		...
		{
			BYTE btMonsterIndex = gMSetBase.m_Mp[n].m_Type;
...
void CChaosCastle::ProcessFallUser(int iChaosCastleIndex)
{
	...
	for ( int iFALL=0;iFALL<MAX_CHAOSCASTLE_USER;iFALL++)
	{
		if ( this->m_stChaosCastleData[iChaosCastleIndex].m_lFallUser[iFALL] != -1 )
		{
			...
			if ( (gObj[iFALL_INDEX].MapNumber) == (iChaosCastleIndex+18) &&
...
int  CChaosCastle::CalcSendRewardEXP(int iUserIndex, int iEXP, int iKILLCOUNT_USER, int iKILLCOUNT_MONSTER)
{
	...
	if ( gObj[iUserIndex].Type == OBJ_USER )
	{
		...
		pkillMsg.h.setE(0x9C, sizeof(pkillMsg));
		pkillMsg.NumberH = -1;
		pkillMsg.NumberL = -1;
...
BOOL CChaosCastle::LevelUp(int iUserIndex, int iAddExp, int iEventType)
{
	...
	else
	{
		...
		if ( gObj[iUserIndex].Class == CLASS_SUMMONER )
		{
			gObj[iUserIndex].LevelUpPoint += 6;
		}
...
BOOL CChaosCastle::ObjSetPosition(int iIndex, int iX, int iY)
{
	...
	PHeadSetB((LPBYTE)&pMove2, GMSG_POSISTION_SET, sizeof(pMove2));
...
void CChaosCastle::SearchNBlowObjs(int iMapNumber, int iX, int iY)
{
	...
	for ( int i=0;i<MAX_CHAOSCASTLE_USER;i++)
	{
		if ( this->m_stChaosCastleData[iMapNumber-18].m_UserData[i].m_iIndex == -1 )
			continue;

		int iIndex = this->m_stChaosCastleData[iMapNumber-18].m_UserData[i].m_iIndex;
		...
		if ( gObj[iIndex].MapNumber == iMapNumber && gObj[iIndex].Connected > PLAYER_LOGGED )
		{
				 ...
				 gObj[iIndex].Y >= iMIN_Y && gObj[iIndex].Y <= iMAX_Y )
			{
				...
				if ( btMapAttr == 8 )
				{
					this->AddFallUser(iMapNumber-18, iIndex);

					LogAddTD("[Chaos Castle] (%d) [%s][%s] User Dead In Chaos Castle : Fall from Castle (X:%d, Y:%d)",
						iMapNumber - 18+1, gObj[iIndex].AccountID, gObj[iIndex].Name, iX, iY);
				}
			}
		}
	}
...
void CChaosCastle::SafetyCastleZone(int iChaosCastleIndex)
{
	...
	for ( int i=g_btCastleSafetyZoneMapXY[iChaosCastleIndex][0] ; i <= g_btCastleSafetyZoneMapXY[iChaosCastleIndex][2] ; i++ )
	{
		...
		{
			MapC[iChaosCastleIndex+18].m_attrbuf[ j * 256 + i] |= 1;
		}
	}
	...
	for ( int iHOLE_NUM = 0; iHOLE_NUM <2;iHOLE_NUM++)
	{
		for(int i=g_btCastleCenterHollowZoneMapXY[iChaosCastleIndex][iHOLE_NUM][0] ; i<= g_btCastleCenterHollowZoneMapXY[iChaosCastleIndex][iHOLE_NUM][2] ; i++ )
		{
			...
			{
				MapC[iChaosCastleIndex+18].m_attrbuf[ j * 256 + i] |= 8;
			}
		}
	}
}
...
void CChaosCastle::SendCastleZoneSafetyInfo(int iChaosCastleIndex, bool bDoSet)
{
	...
	for (int i=OBJ_STARTUSERINDEX;i<OBJMAX;i++)
	{
		if ( gObj[i].MapNumber == (iChaosCastleIndex + 18) )
		{
...
void CChaosCastle::CheckMonsterInDieTile(int iChaosCastleIndex)
{
	...
	for ( int iAXIS =0;iAXIS <	4 ; iAXIS++)
	{
		for(int iMAPX = g_rtPOINT_TRAP[iAXIS].left; iMAPX <= g_rtPOINT_TRAP[iAXIS].right ; iMAPX++)
		{
			for(int iMAPY = g_rtPOINT_TRAP[iAXIS].top; iMAPY <= g_rtPOINT_TRAP[iAXIS].bottom ; iMAPY++)
			{
				BYTE btMapAttr = MapC[iChaosCastleIndex+18].GetAttr(iMAPX, iMAPY);
...
void CChaosCastle::CheckUserInDieTile(int iChaosCastleIndex)
{
	...
	for ( int i=0;i<MAX_CHAOSCASTLE_USER;i++)
	{
		...
		{
			if ( gObjIsConnected(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex) ==TRUE &&
				 gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].MapNumber == (iChaosCastleIndex + MAP_INDEX_CHAOSCASTLE1) )
			{
				 ...
				 gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].MapNumber == (iChaosCastleIndex + MAP_INDEX_CHAOSCASTLE1) )
			{
					   ...
					   iSY > g_iChaosCastle_DamageAxis[iTRAP_STEP][3] ) )
				{
					...
					if ( gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].m_ColdBeattackCount <= 0 )
					{
						...
						GCStateInfoSend(&gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex], 1,
							eVS_ICE);
					}
				}
			}
...
void CChaosCastle::GiveWinnerItem(int iChaosCastleIndex, int iWinnerIndex)
{
	...
	wsprintf(szTemp,"[Chaos Castle %d] Winner is %s",iChaosCastleIndex+1,gObj[iWinnerIndex].Name);
	AllSendServerMsg(szTemp);

	int Count = Role(1,1);
	for(int i = 0 ; i != Count ; i++)
	{
		int rand_index = Role(0,this->m_iCC_GIFTS_COUNT);

		int itemnum = ItemGetNumberMake( this->m_Gifts[rand_index].SubType , this->m_Gifts[rand_index].Type);
		int iItemLevel = this->m_Gifts[rand_index].Level;
	
		ItemSerialCreateSend(gObj[iWinnerIndex].m_Index, gObj[iWinnerIndex].MapNumber,
			gObj[iWinnerIndex].X, gObj[iWinnerIndex].Y, itemnum, iItemLevel, 255, 0, 0, 0, iWinnerIndex, 0, 0);
		...
		if ( this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex != -1 )
		{
			if ( gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].Connected == PLAYER_PLAYING )
			{
				...
				LogAddTD("[Chaos Castle][Bug Tracer] (%d) [%s][%s] Reword ZettoMoney(Map:%d) Send Message",
					iChaosCastleIndex+1, gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].AccountID,
					gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].Name,
					gObj[iZettoWinnerIndex].MapNumber - 18);
			}
		}
	}
