		...
		while ( true )
		{
			...
			while ( true )
			{
				...
				if ( iType == 2 )
				{
					...
					if ( iGroupNumber <= -1 || iGroupNumber >= MAX_CRYWOLF_MONSTER_GROUP )	// #error Change && to ||
					{
...
void CCrywolf::SetCrywolfCommonNPC(int iOccupationState)
{
	for ( int i=0;i<this->m_ObjCommonNPC.m_iObjCount ; i++)
	{
		...
		{
			lpObj->m_ViewSkillState[eVS_CRYWOLF_COMMON_NPC] = 1;
			GCStateInfoSend(lpObj, 1, eVS_CRYWOLF_COMMON_NPC);
		}
		else if ( iOccupationState == 0 )
		{
			lpObj->m_ViewSkillState[eVS_CRYWOLF_COMMON_NPC] = 0;
			GCStateInfoSend(lpObj, 1, eVS_CRYWOLF_COMMON_NPC);
		}
...
void CCrywolf::SetCrywolfAllCommonMonsterState(int iMonsterState, int iMode)
{
	for ( int i=0;i<this->m_ObjCommonMonster.m_iObjCount ; i++)
	{
		...
		if ( iMode == 0 )
		{
			lpObj->m_ViewSkillState[iMonsterState] = 0;
		    GCStateInfoSend(lpObj, iMode, iMonsterState);
		}
		else if ( iMode == 1 )
		{
			lpObj->m_ViewSkillState[iMonsterState] = 1;
		    GCStateInfoSend(lpObj, iMode, iMonsterState);
		}
	}
}
