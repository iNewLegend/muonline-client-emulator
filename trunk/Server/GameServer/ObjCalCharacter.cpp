...
void gObjCalCharacter(int aIndex)
{
	...
	if ( lpObj->Class == CLASS_ELF ) // Elf
	{
			  ...
			  Left->m_Type == ITEMGET(4,17) )
		{
			...
			else
			{
				...
				lpObj->m_AttackDamageMaxRight = (Dexterity / 3) + (Strength / 8 );
				lpObj->m_AttackDamageMinLeft = (Dexterity / 7) + (Strength / 14);
				lpObj->m_AttackDamageMaxLeft = (Dexterity / 3) + (Strength / 8 );
			}
		}
		else
		{
	...
	{
		lpObj->m_AttackDamageMinRight = (Strength / 5) + (Energy / 7);
		lpObj->m_AttackDamageMaxRight = (Strength / 3) + (Energy / 5);
		lpObj->m_AttackDamageMinLeft = (Strength / 5) + (Energy / 7);
		lpObj->m_AttackDamageMaxLeft = (Strength / 3) + (Energy / 5);
	...
	else if (lpObj->Class == CLASS_SUMMONER ) // Freiya
	{
		lpObj->m_AttackDamageMinRight = (Strength / 5) + (Energy / 6);
		lpObj->m_AttackDamageMaxRight = (Strength / 3) + (Energy / 4);
		lpObj->m_AttackDamageMinLeft = (Strength / 5) + (Energy / 6);
		lpObj->m_AttackDamageMaxLeft = (Strength / 3) + (Energy / 4);
	}
	else
	{
	...
	{
		lpObj->m_AttackSpeed = Dexterity / 40;
		lpObj->m_MagicSpeed = Dexterity / 40;
	}
	...
	if ( lpObj->pInventory[8].IsItem() != FALSE )
	{
		...
		{
			lpObj->m_Defense += INT( Dexterity / 10 + 5 + Helper->m_PetItem_Level * 3 );
		}
	}
	...
	if ( lpObj->pInventory[8].m_Type == ITEMGET(13,37) && lpObj->pInventory[8].m_NewOption == 4 ) //Golden Fenrir Attribute
	{
	...
	if ( lpObj->pInventory[7].m_Type >= ITEMGET(12,0) && lpObj->pInventory[7].m_Type <= ITEMGET(12,6) ) //Third Wings Fix Applied
	{
...
#define GET_MAX_RESISTANCE(x,y,z) ( ( ( ( (x) > (y) ) ? (x) : (y) ) > (z) ) ? ( ( (x) > (y) ) ? (x) : (y) ) : (z) )	
	...
	if ( lpObj->Class == CLASS_KNIGHT || lpObj->Class == CLASS_MAGUMSA || lpObj->Class == CLASS_DARKLORD || lpObj->Class == CLASS_SUMMONER  )
	{
