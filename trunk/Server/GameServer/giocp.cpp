...
CLogToFile SERVER_MSG_LOG("SERVER_MSG_LOG", ".\\SERVER_MSG_LOG", 1);



void LogAddClientMsgInfo1(int aIndex, unsigned char * lpMsg)
{
	int nProtoNum = lpMsg[2];
	int nSubProtoNum= lpMsg[3];
	int nSize = lpMsg[1];
	if (lpMsg[0]==0xC2||lpMsg[0]==0xC4)
	{
		nProtoNum = lpMsg[3];
		nSubProtoNum= lpMsg[4];
		nSize = (lpMsg[1]<<8)+lpMsg[2];
	}
	char szTemp[1024]={0};
	for (size_t i=0;i<nSize;++i)
	{
		wsprintf(szTemp,"%s%2X ",szTemp,lpMsg[i]);
	}
	if (nProtoNum!=GMSG_MOVE&&nProtoNum!=GMSG_WEATHER&&nProtoNum!=GMSG_ACTION&&nProtoNum!=GMSG_ATTACK&&
		nProtoNum!=GMSG_CLIENT_TIME&&nProtoNum!=GMSG_MONSTER_VIEWPORT_CREATE&&nProtoNum!=GMSG_VIEWPORT_DESTROY&&
		nProtoNum!=GMSG_REFILL&&nProtoNum!=GMSG_MANA_SEND)
	{
		LogAddC(3,"SEND MSG [%s] %X:%X_%X size=%d",gObj[aIndex].AccountID, lpMsg[0], nProtoNum, nSubProtoNum, nSize);
	}
	SERVER_MSG_LOG.Output("SEND MSG [%s] %X:%X_%X size=%d [%s]",gObj[aIndex].AccountID, lpMsg[0], nProtoNum, nSubProtoNum, nSize, szTemp);
}

void LogAddClientMsgInfo(int aIndex, unsigned char * lpMsg)
{
	int nProtoNum = lpMsg[2];
	int nSubProtoNum= lpMsg[3];
	int nSize = lpMsg[1];
	...
	if (nProtoNum!=GMSG_MOVE&&nProtoNum!=GMSG_WEATHER&&nProtoNum!=GMSG_ACTION&&nProtoNum!=GMSG_ATTACK&&
		...
		nProtoNum!=GMSG_REFILL&&nProtoNum!=GMSG_MANA_SEND)
	{
		LogAddC(7,"RECV MSG [%s] %2X:%2X_%2X size=%d [%s]",gObj[aIndex].AccountID, lpMsg[0], nProtoNum, nSubProtoNum, nSize,szTemp);
	}
	SERVER_MSG_LOG.Output("RECV MSG [%s] %2X:%2X_%2X size=%d [%s]",gObj[aIndex].AccountID, lpMsg[0], nProtoNum, nSubProtoNum, nSize,szTemp);
}
		...
		if ( size <= lpIOContext->nSentBytes )
		{
...
#ifdef _DEBUG
					LogAddClientMsgInfo(uIndex, byDec);
#endif
					ProtocolCore(headcode, byDec, byDec[1]+1, uIndex, 1, serial);
					//return;
				}
			}
			else if ( xcode == 0xC4 )
			{
				...
				else
				{
					...
					LogAddClientMsgInfo(uIndex, byDec);
#endif
					ProtocolCore(headcode, byDec, ret, uIndex, 1, serial);
					//continue;
				}
			}
			else
...
{
#ifdef _DEBUG
	LogAddClientMsgInfo1(aIndex,lpMsg);
#endif
