...
#define KILLCOUNT_DECREASE	2
	...
	enum eVIEWSKILL
	{
		...
		eVS_22 = 22, // 0x20000
		eVS_23 = 23,
		eVS_ALTAR_OF_WOLF_CONTRACT_ATTEMPT                        = 24, // 0x80000
		eVS_ADMIN                                                 = 25,
		eVS_ALTAR_OF_WOLF_HERO_CONTRACT_STATUS                    = 26, // 0x40000
		eVS_INVISIBLE             = 27,
		eVS_GAMEMASTER = 28,
...
#define MAX_VIEWSKILL	200	/*0x57*/


#if (GS_CASTLE)
#define OBJMAX 4000
#define OBJMAXUSER 10
#define OBJ_MAXMONSTER 2000
#else
#define OBJMAX 9600			//GS-CS 4000 Normal 9600
#define OBJMAXUSER 10		//GS-CS 1500 Normal 1000
#define OBJ_MAXMONSTER 8000	//GS-CS 2500 Normal 8000
#endif
...
#define MAX_MAGIC 60
#define MAX_VIEWPORT 90			// ORGINAL 75

#if (GS_CASTLE == 1 )
#define MAX_VIEWPORT_MONSTER 75
#else
#define MAX_VIEWPORT_MONSTER 50 // ORGINAL 20
#endif
...
{
	/* Size: 50 BYTES */
	BYTE m_DoneQuests[MAX_CHAR_QUESTS]; /* 0 */
	BYTE b_CurrentQuest; /* 20 */
	BYTE b_CurrentSubQuest; /* 21 */
	BYTE b_State; /* 22 */
	BYTE m_Info[40 - MAX_CHAR_QUESTS]; /* 23 */
	BYTE m_Extra[10]; /* 40 */
};
...
struct OBJECTSTRUCT
{
	...
	BYTE m_ViewSkillState[MAX_VIEWSKILL];	// 1E4
	...
	int m_IllusionIndex; // 196C
	int m_IllusionTeam; // 1970
	int m_IllusionKillCount; // 1974
	int m_IllusionShieldSkillTime; // 1978
...
struct OBJECTSTRUCT
{
  ...
  /*<thisrel this+0x1e4>*/ /*|0x4|*/ BYTE m_ViewSkillState[MAX_VIEWSKILL];
	...
	BYTE m_btDestX;	// 1524
	BYTE m_btDestY;	// 1525
	BYTE m_btCsNpcExistVal1; // 1528
	BYTE m_btCsNpcExistVal2; // 1529
    BYTE m_btCsNpcExistVal3; // 152A
    BYTE m_btCsNpcExistVal4; // 152B
	int m_iCsNpcExistVal; // 1528
    BYTE m_btCsNpcType; // 152C
    BYTE m_btCsGateOpen; // 152D
    int m_iCsGateLeverLinkIndex; // 1530
    BYTE m_btCsNpcDfLevel; // 1534
    BYTE m_btCsNpcRgLevel; // 1535
    BYTE m_btCsJoinSide; // 1536
    bool m_bCsGuildInvolved; // 1537
	bool m_bIsCastleNPCUpgradeCompleted;	// 1538
	BYTE m_btWeaponState;	// 1539
	int m_iWeaponUser;	// 153C
	BYTE m_btKillCount;	// 1540
	int m_iSkillStunTime;	// 1544
	int m_iSkillBrandOfSkillTime;	// 1548
	int m_iSkillInvisibleTime;	// 154C
	int m_iSkillManaSwellTime;	// 1550
	int m_iSkillManaSwell;	// 1554
	int m_iAccumulatedDamage;	// 1558
	int m_iPotionBlessTime;	// 155C
	int m_iPotionSoulTime;	// 1560
	BYTE m_btLifeStoneCount; // 1564
    BYTE m_btCreationState; // 1565
	int m_iCreatedActivationTime; // 1568
	int m_iAccumulatedCrownAccessTime;	// 156C
	TMonsterSkillElementInfo m_MonsterSkillElementInfo;	// 1570
	int m_iBasicAI;	// 15C0
	int m_iCurrentAI;	// 15C4
	int m_iCurrentAIState;	// 15C8
	int m_iLastAIRunTime;	// 15CC
	int m_iGroupNumber;	// 15D0
	int m_iSubGroupNumber;	// 15D4
	int m_iGroupMemberGuid;	// 15D8
	int m_iRegenType;	// 15DC
	TMonsterAIAgro m_Agro;	// 15E0
	int m_iLastAutomataRuntime;	// 1904
	int m_iLastAutomataDelay;	// 1908
	int m_iCrywolfMVPScore;	// 190C
	DWORD m_dwLastCheckTick;	// 1910
	int m_iAutoRecuperationTime;	// 1914
	int m_iSkillNPCHelpTime;	// 1918
	int m_iSkillNPCDefense;	// 191C
	int m_iSkillNPCAttack;	// 1920
	int m_iMuseElfInfinityArrowSkillTime;	// 1924
	int m_iSkillDistanceErrorCount;	// 1928
	DWORD m_dwSkillDistanceErrorTick;	// 192C
	JEWELOFHARMONY_ITEM_EFFECT m_JewelOfHarmonyEffect;	// 1930
	ITEMOPTION_FOR380ITEM_EFFECT m_ItemOptionExFor380;	// 1956
	BOOL m_bKanturuEntranceByNPC;	// 194C
	WORD m_wItemEffectType;	// 1966
	int m_iItemEffectValidTime;	// 1968
	WORD m_wCashPoint;	// 196C
	int m_iPeriodItemEffectIndex;	// 1970
	WORD m_wExprienceRate;	// 1974
	WORD m_wItemDropRate;	// 1976
	BYTE m_btMoveMapBound;	// 1978
	BYTE m_btSealIndex;  // 1968
  ITEM_SOCKET_EFFECT ItemSocketEffect;
  int PCPoints;
	//float PCPoints;
...
int gObjLevelUpPointAdd(unsigned char type, unsigned int uPointCount, LPOBJ lpObj);
...
int gObjGetIndexByName(char* Name);//leo123
