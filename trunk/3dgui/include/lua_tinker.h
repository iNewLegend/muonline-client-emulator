...
template<typename A, typename B>		struct if_<false, A, B> { typedef B type; };
	...
	struct class_
	{
		...
		class_(lua_State* L, const char* name) 
			: m_L(L)
		{
			class_add<T>(m_L, name);
		}
		// constructor
		template<typename CONSTRUCTOR>
		class_<T>& con(CONSTRUCTOR)
		{
			meta_push(m_L, class_name<T>::name());
		...
		{ 
			class_def<T>(m_L, name, func);
			return *this; 
		}
		// variables
		template<typename BASE, typename VAR>
		class_<T>& mem(const char* name, VAR BASE::*val) 
		{
			class_mem<T>(m_L, name, val);
			return *this; 
		}
		//
		lua_State* m_L;  
