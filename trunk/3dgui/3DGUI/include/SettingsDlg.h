...
public:
    ...
    void Init( CUIDialogResMgr* pManager );
    void Init( CUIDialogResMgr* pManager, LPCWSTR szControlTextureFileName );
	void Init( CUIDialogResMgr* pManager, LPCWSTR pszControlTextureResourcename, HMODULE hModule);
    ...
    CUIDialog* GetDialogControl() { return &m_Dialog; }
...
protected:
    ...
    void OnEvent( UINT nEvent, int nControlID, CUIControl* pControl );
    static void WINAPI StaticOnEvent( UINT nEvent, int nControlID, CUIControl* pControl, void* pUserData );
