...
//--------------------------------------------------------------------------------------
void CD3DSettingsDlg::Init( CUIDialogResMgr* pManager )
{
    assert( pManager );
...
//--------------------------------------------------------------------------------------
void CD3DSettingsDlg::Init( CUIDialogResMgr* pManager, LPCWSTR szControlTextureFileName )
{
    assert( pManager );
...
//--------------------------------------------------------------------------------------
void CD3DSettingsDlg::Init( CUIDialogResMgr* pManager, LPCWSTR pszControlTextureResourcename, HMODULE hModule )
{
    assert( pManager );
...
void CD3DSettingsDlg::CreateControls()
{
   ...
   // CDXUTElement* pStyle = m_Dialog.GetDefaultElement( DXUT_CONTROL_STATIC, 0 );
   // if( pStyle )
    {
      //  pStyle->dwTextFormat = DT_VCENTER | DT_RIGHT;
        
        // Title
        CDXUTStatic* pStatic = NULL;
        m_Dialog.AddStatic( DXUTSETTINGSDLG_STATIC, L"Direct3D设置", 10, 5, 400, 50, false, &pStatic );
    //    pStyle = pStatic->GetElement( 0 );
    //    pStyle->iFont = 1;
    //    pStyle->dwTextFormat = DT_TOP | DT_LEFT;
    }

    // DXUTSETTINGSDLG_ADAPTER
    m_Dialog.AddStatic( DXUTSETTINGSDLG_STATIC, L"显卡", 10, 50, 180, 23 );//Display Adapter
    m_Dialog.AddComboBox( DXUTSETTINGSDLG_ADAPTER, 200, 50, 300, 23 );

    // DXUTSETTINGSDLG_DEVICE_TYPE
    m_Dialog.AddStatic( DXUTSETTINGSDLG_STATIC, L"渲染模式", 10, 75, 180, 23 );//Render Device
    m_Dialog.AddComboBox( DXUTSETTINGSDLG_DEVICE_TYPE, 200, 75, 300, 23 );

    // DXUTSETTINGSDLG_WINDOWED, DXUTSETTINGSDLG_FULLSCREEN
    m_Dialog.AddRadioButton( DXUTSETTINGSDLG_WINDOWED, DXUTSETTINGSDLG_WINDOWED_GROUP, L"窗口模式", 240, 105, 300, 16 );
    m_Dialog.AddCheckBox( DXUTSETTINGSDLG_DEVICECLIP, L"当 窗口 一段时间 与...相交叉 多人享有 监视器 时 删去设备 ", 250, 126, 400, 16 );//Clip to device when window spans across multiple monitors
    m_Dialog.AddRadioButton( DXUTSETTINGSDLG_FULLSCREEN, DXUTSETTINGSDLG_WINDOWED_GROUP, L"全屏模式", 240, 147, 300, 16 );

    // DXUTSETTINGSDLG_ADAPTER_FORMAT
    m_Dialog.AddStatic( DXUTSETTINGSDLG_ADAPTER_FORMAT_LABEL, L"显示格式", 10, 180, 180, 23 );//Adapter Format
    m_Dialog.AddComboBox( DXUTSETTINGSDLG_ADAPTER_FORMAT, 200, 180, 300, 23 );

    // DXUTSETTINGSDLG_RESOLUTION
	m_Dialog.AddStatic( DXUTSETTINGSDLG_RESOLUTION_LABEL, L"分辨率", 10, 205, 180, 23 );
...
HRESULT CD3DSettingsDlg::Refresh()
{
    ...
    //m_Dialog.Refresh();
    CUIDialog::SetRefreshTime( (float) DXUTGetTime() );
...
HRESULT CD3DSettingsDlg::OnResetDevice()
{
    ...
    m_Dialog.SetBackgroundColors( D3DCOLOR_ARGB(255, 138, 138, 138), 
                                         D3DCOLOR_ARGB(255, 138, 138, 138),
                                         D3DCOLOR_ARGB(255, 64, 64, 98),
                                         D3DCOLOR_ARGB(255, 64, 64, 98) );
...
void WINAPI CD3DSettingsDlg::StaticOnEvent( UINT nEvent, int nControlID, 
                                            CUIControl* pControl, void* pUserData )
{
...
HRESULT CD3DSettingsDlg::OnBackBufferFormatChanged()
{
    ...
    for( int idc=0; idc < pDeviceInfo->deviceSettingsComboList.GetSize(); idc++ )
    {
            ...
            pDeviceCombo->BackBufferFormat == backBufferFormat )
        {
            ...
            pPresentIntervalComboBox->AddItem( L"打开", ULongToPtr(D3DPRESENT_INTERVAL_DEFAULT) );
            pPresentIntervalComboBox->AddItem( L"关闭", ULongToPtr(D3DPRESENT_INTERVAL_IMMEDIATE) );
