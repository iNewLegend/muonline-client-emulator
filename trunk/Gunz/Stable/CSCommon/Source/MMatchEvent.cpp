...
void MMatchEventDescManager::ClearEventDesc()
{
	MMatchEventDescManager::iterator iter = MMatchEventDescManager::begin();
	for (; iter != MMatchEventDescManager::end(); ++iter)
	{
		delete iter->second;
	}
...
void MMatchProbabiltyEventPerTime::OnRun()
{
	...
	MAsyncDBJob_EventLog* pEvnJob = new MAsyncDBJob_EventLog;
	if( 0 != pEvnJob )
	{
...
void MMatchUniquePlayerListEvent::OnCheckEventObj( MMatchObject* pObj, const DWORD dwCurTime )
{
	map< MUID, DWORD >::iterator itFind = m_UniquePlayerList.find( pObj->GetUID() );
	if( m_UniquePlayerList.end() != itFind )
		return;

	m_UniquePlayerList.insert( pair<MUID, DWORD>(pObj->GetUID(), dwCurTime) );
	m_vEventObj.push_back( pObj->GetUID() );
}


void MMatchUniquePlayerListEvent::OnRun()
{
	if( m_vEventObj.empty() ) 
		return;
