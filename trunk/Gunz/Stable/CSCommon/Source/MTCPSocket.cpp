		...
		while(TRUE) {
			...
			if (nSent == SOCKET_ERROR) {
				int nErrCode = WSAGetLastError();
					#ifdef _DEBUG
					char szBuf[64]; sprintf(szBuf, "FLUSHSEND> FlushSend Error(%d)\n", nErrCode);
...
#ifdef _DEBUG
			char szLog[64];
			sprintf(szLog, "Connect Failed (ErrorCode=%d) \n", dwError);
