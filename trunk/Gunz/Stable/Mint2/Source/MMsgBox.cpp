...
void MMsgBox::OnSize(int w, int h)
{
	...
	int nOKOffset = 0;
	if(m_nType==MT_OKCANCEL || m_nType==MT_YESNO) 
		nOKOffset = MMSGBOX_OK_W+ CONVERT800(5);

	if(m_pOK!=NULL) m_pOK->SetBounds(MRECT(
		r.x+r.w-MMSGBOX_OK_W-nOKOffset - CONVERT800(19), 
		r.y+r.h-MMSGBOX_OK_H-CONVERT600(20), 
		MMSGBOX_OK_W, 
		MMSGBOX_OK_H));
	
	if(m_pCancel!=NULL) m_pCancel->SetBounds(MRECT(
		r.x+r.w-MMSGBOX_OK_W - CONVERT800(19), 
		r.y+r.h-MMSGBOX_OK_H-CONVERT600(20), 
		MMSGBOX_OK_W, 
		MMSGBOX_OK_H));
#endif
...
void MMsgBox::MultiplySize( float byIDLWidth, float byIDLHeight, float byCurrWidth, float byCurrHeight )
{
