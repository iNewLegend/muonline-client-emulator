...
//			SetMtrlDiffuse(pMtrl,vis_alpha);
		...
		if( pMtrl->m_bAlphaTestMap ) {
			...
			int nAlphaRef = pMtrl->m_nAlphaTestValue;
			nAlphaRef *= vis_alpha;
			// 메시를 반투명으로 그린다면 원래 알파를 갖고있던 부위는 알파값이 더 낮아져서
			// alphatest를 통과못하는 현상이 발생 (특정부위가 통째로 안그려짐).
			// 따라서 반투명을 적용하려면 alphatest 기준치도 같은 비율만큼 낮춰주기로한다
			SetShaderAlphaTestMap( nAlphaRef,vis_alpha );
		}
...
//			SetShaderNormalMap();
	...
	//////////////////////////////////////////////////////////////////////////
	// 2-pass alpha 일때 스테이트 처리
	if( m_eRenderMode==PASS_DEPTH_ONLY )
	{
//		dev->SetTexture( 0, NULL);
	...
	else if( m_eRenderMode==PASS_COLOR )
	{
//		dev->SetRenderState(D3DRS_SRCBLEND, D3DBLEND_SRCALPHA); 
//		dev->SetRenderState(D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA); 
