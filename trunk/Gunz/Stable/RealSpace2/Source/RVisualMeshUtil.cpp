...
void RWeaponTracks::Create(int size)
{
	...
	m_last_addvertex_time = timeGetTime();
}
...
bool RWeaponTracks::IsTimeToAddVertex()
{
	const DWORD ADDVERTEX_INTERVAL = 17;		// 17ms마다, 즉 초당 약 60번 업데이트

	DWORD now_time = timeGetTime();
	DWORD elapsed_time = now_time - m_last_addvertex_time;

	if (elapsed_time >= ADDVERTEX_INTERVAL)
	{
		m_last_addvertex_time = now_time;
		return true;
	}
	return false;
}
