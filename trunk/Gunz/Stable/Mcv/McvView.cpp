...
bool	g_bCameraLight = false;			// 성환씨 요청으로 카메라 방향에 따라 캐릭터 밝기를 최대로 볼수있게 처리
...
*/
		if (!pVMesh->ChangeChestCloth(1.f,1))
		{
			...
			{
				AfxMessageBox("현재 CHEST 파츠에는 vertex color가 있습니다.\n따라서 cloth mesh로 인식하였지만 bone이나 physique을 찾을 수 없습니다.\n\nCHEST에 bone 또는 physique이 누락되었거나, 의도치 않은 vertex color가 들어갔는지 확인해주세요.", 
					MB_OK|MB_ICONEXCLAMATION);
				__asm int 3	// 그냥 죽이자 -_-
			}
		}
...
void CMcvView::OnEffectRender()
{
	...
	// 캐릭터를 로딩했을때는 처리를 해주지 말아야 한다.
	// 캐릭터는 버텍스 세팅이 다르기 때문에 EffectRender를 켜주면 버텍스가 깨져서 출력됨
}
