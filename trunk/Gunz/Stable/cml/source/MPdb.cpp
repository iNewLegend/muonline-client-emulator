...
//			mlog("LoadModuleSymbols,INVALID_HANDLE_VALUE\n");
		...
		while (fFlag)
		{
			...
			if(strnicmp(module.szModule,"d3d9",4)!=0) {
...
// string객체의 메모리 할당에서 실패 예외가 발생할 수 있어서 
// 파일에 바로 쓰는 WriteCrashInfo함수를 사용하자.
...
//
//			_snprintf(t_temp, 1024, "SymGetSymFromAddr error %d\n",GetLastError());str += t_temp;			
//		}
//
//		nframes++;
//
//		_snprintf(t_temp, 1024, "frame : (%02d) : PC Address : %08x\n", nframes, stk.AddrPC.Offset);
//
//		str += t_temp;
//
...
//		{
//			_snprintf(t_temp, 1024, "     ModuleName : %s\n", mi.ModuleName);	str += t_temp;
//		}
//		else {
//			_snprintf(t_temp, 1024, "SymGetModuleInfo error %d\n",GetLastError());str += t_temp;			
//		}
//
//		dwDisplacement = 0;
//
...
//			{
//				_snprintf(t_temp, 1024, "     File Name : %s\n", imageLine.FileName);		str += t_temp;
//				_snprintf(t_temp, 1024, "     Line Number : %d\n", imageLine.LineNumber);	str += t_temp;
//			}else {
//				_snprintf(t_temp, 1024, "SymGetLineFromAddr error %d\n",GetLastError());str += t_temp;			
//			}
//		}
//
//		_snprintf(t_temp, 1024, "     Function Name : %s\n", szSymName);			str += t_temp;
//
//		_snprintf(t_temp, 1024, "\n\n");
//
...
//			::MessageBox(NULL,"SymGetSymFromAddr failed","error",MB_OK);
		...
		sprintf(t_temp,"     Param[0] : %08x\n", stk.Params[0]);		fprintf( fpLog, t_temp );
		sprintf(t_temp,"     Param[1] : %08x\n", stk.Params[1]);		fprintf( fpLog, t_temp );
		sprintf(t_temp,"     Param[2] : %08x\n", stk.Params[2]);		fprintf( fpLog, t_temp );
		sprintf(t_temp,"     Param[3] : %08x\n", stk.Params[3]);		fprintf( fpLog, t_temp );
