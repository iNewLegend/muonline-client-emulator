...
void CHMAC_SHA1::HMAC_SHA1_messege(BYTE *text, int text_len, BYTE *key, int key_len, BYTE *digest)
{
...
void CHMAC_SHA1::HMAC_SHA1_file(char* fileName, BYTE *key, int key_len, BYTE *digest)
{
	memset(SHA1_Key, 0, SHA1_BLOCK_SIZE);
	...
	Update((UINT_8 *)m_ipad, sizeof(m_ipad));	
	if( !CSHA1::HashFile(fileName))
	{
		return;
	}
