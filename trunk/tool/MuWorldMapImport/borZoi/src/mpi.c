...
{
  return (int)((double)bits * LOG_V_2(r)) + 3;
...
mp_err   s_mp_and(mp_int *a, mp_int *b)        /* logical AND */
{
  mp_digit *pa, *pb;
  mp_size   ix, used;
  mp_err    res;
  ...
  used = USED(b);
  /* Make sure a has enough precision for the output value */
  if((USED(b) > USED(a)) && (res = s_mp_pad(a, used)) != MP_OKAY)
    return res;

  /*
    AND all digits up to the precision of b.
   */
  pa = DIGITS(a);
  pb = DIGITS(b);
  for(ix = 0; ix < used; ++ix) {
  ...
  /* If we run out of 'b' digits before we're actually done, make
     sure the rest of 'a' is set to 0.
   */
  used = USED(a)-USED(b);
  for(ix = 0; ix < used; ++ix) {
    *pa++ = 0;
...
mp_err   s_mp_or(mp_int *a, mp_int *b)        /* logical OR */
{
  mp_digit *pa, *pb;
  mp_size   ix, used;
  mp_err    res;
  ...
  /*
    OR all digits up to the precision of b.
   */
  pa = DIGITS(a);
  pb = DIGITS(b);
  for(ix = 0; ix < used; ++ix) {
...
mp_err   s_mp_xor(mp_int *a, mp_int *b)        /* logical XOR */
{
  mp_digit *pa, *pb;
  mp_size   ix, used;
  mp_err    res;
  ...
  /*
    XOR all digits up to the precision of b.
   */
  pa = DIGITS(a);
  pb = DIGITS(b);
  for(ix = 0; ix < used; ++ix) {
...
mp_err   s_mp_gen_random(mp_int *a, unsigned long n)   /* generate a random mp_int of n bits*/
{
  mp_digit *pa;
  mp_size   ix, prec;
  ...
  pa = DIGITS(a);
  for(ix = 0; ix < prec; ++ix) {
    *pa++ = (unsigned short)gen_random();
  }
  pa--;
  *pa >>= (MP_DIGIT_BIT-(n%MP_DIGIT_BIT));

  return MP_OKAY;

} /* end s_mp_gen_random() */

unsigned char s_mp_tooctet(mp_int *a)
{
  mp_digit *pa;

  pa = DIGITS(a);
  return (unsigned char)(*pa&0xff);
...
/*
  Returns the most significant bit of v or -1 if v is zero.
 */
int      mp_msb(mp_int *v)
{
  mp_digit d;
  ...
  mp_size   du = USED(v) - 1;
  mp_digit *zp = DIGITS(v) + du;

  while(du > 0 && !*zp--)
    --du;

  d = DIGIT(v, du); /* most significant digit of v */
...
/*
  Returns the bit n of v.
 */
int      mp_getbit(mp_int *v, unsigned long n)
{
  mp_digit *pv = DIGITS(v);

  if (n>(USED(v)*DIGIT_BIT))
	  return 0x0;
  else return ((pv[n/DIGIT_BIT]>>(n%DIGIT_BIT))&0x1);
...
mp_err   s_poly_F2x_div(mp_int *a, mp_int *b)
{
  mp_int   quot, rem, t, tb;
  mp_err   res;
  mp_digit      msb_r, msb_b, bit_cnt;
  ...
  /* the quotient */
  if((res = mp_init(&quot)) != MP_OKAY)
    return res;

  /* working temporaries for division     */
  if((res = mp_init(&t)) != MP_OKAY)
    goto T;

  if((res = mp_init(&tb)) != MP_OKAY)
    goto TB;
  ...
  bit_cnt = msb_r - msb_b;
  while ((msb_r>=msb_b)) {
	  /*printf ("b:   "); mp_print (b, stdout); printf ("\n");*/
	  mp_copy (b, &tb);
	  if((res = mp_left_shift(&tb, (msb_r-msb_b))) != MP_OKAY) 
		  goto CLEANUP; 
	  /*printf ("tb:  "); mp_print (&tb, stdout); printf ("\n");*/
	  
	  s_mp_2expt(&t, msb_r-msb_b);
	  /*printf ("t:   "); mp_print (&t, stdout); printf ("\n");*/
	  s_mp_or(&quot, &t);
	  /*printf ("quot:"); mp_print (&quot, stdout); printf ("\n");*/

	  /*printf ("rem: "); mp_print (&rem, stdout); printf ("\n");*/
	  s_mp_xor (&rem, &tb); 
	  /*printf ("rem: "); mp_print (&rem, stdout); printf ("\n");*/
...
mp_err mp_gen_random(mp_int *a, unsigned long n)
{
  ...
  if((res = s_mp_gen_random(a, n)) != MP_OKAY)
	  return res; 
...
{
  return s_mp_tooctet(a);

} /* end mp_tooctet() */


/* {{{ poly_F2x_div(a, b, q, r) */
...
mp_err poly_F2x_div(mp_int *a, mp_int *b, mp_int *q, mp_int *r)
{
  ...
  if((res = s_poly_F2x_div(&qtmp, &rtmp)) != MP_OKAY)
    goto CLEANUP;
...
mp_err poly_F2x_mulmod(mp_int *a, mp_int *b, mp_int *m, mp_int *c)
{
  ...
  if((res = F2x_multiply(a, b, c)) != MP_OKAY)
    return res;
  if((res = poly_F2x_mod(c, m, c)) != MP_OKAY)
    return res;
...
mp_err poly_F2x_mod(mp_int *a, mp_int *m, mp_int *c)
{
  ...
  if(s_mp_cmp(a, m) > 0) {
    if((res = poly_F2x_div(a, m, NULL, c)) != MP_OKAY)
      return res;
...
mp_err F2x_multiply(mp_int *a, mp_int *b, mp_int *c)
{
  ...
  mp_err  res;
  mp_int b_tmp, c_tmp;
  mp_size  i, j, t = USED(a);

  ARGCHK(a != NULL && b != NULL && c != NULL, MP_BADARG);
  if((res = mp_init(&c_tmp)) != MP_OKAY)
    return res;
  if((res = mp_init(&b_tmp)) != MP_OKAY)
    goto C_CLEANUP;

  mp_copy (b, &b_tmp);
  mp_zero (&c_tmp); /* c <- 0 */ 
  s_mp_pad (&c_tmp, USED(&c_tmp)+USED(a)+USED(b)+1);
...
/*
  ...
  Algorithm 7 in "Software Implementation of Elliptic Curve Cryptography
  Over Binary Fields", D. Hankerson, J.L. Hernandez, A. Menezes.
 
 */

static const mp_digit T[] = {
...
mp_err F2m_square(mp_int *a, mp_int *f, mp_int *b)
{
  ...
  ARGCHK(a != NULL && b != NULL && f != NULL, MP_BADARG);
  s_mp_pad (b, 2*t);
  ...
  for (i=0; i<t; i++) {
	  pb[2*i] = T[pa[i]&0xff];
	  pb[2*i+1] = T[(pa[i]>>8)&0xff];
  }

  s_mp_clamp (b);
  return poly_F2x_mod(b, f, b);
} /* end F2x_square() */
...
mp_err F2m_inverse(mp_int *a, mp_int *f, mp_int *b)
{
  ...
  ARGCHK(a != NULL && f != NULL && b != NULL, MP_BADARG);
  ...
  while (mp_msb(&u)!=0) { /* deg(u) != 0 */
	  j = mp_msb(&u)-mp_msb(&v); /* j <- deg(u) - deg(v) */
	  if (j<0) {
		  s_mp_exch (&u, &v); /* u <-> v */
		  s_mp_exch (b, &c); /* b <-> c */
		  j = -j; /* j <- -j */
