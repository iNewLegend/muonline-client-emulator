...
#ifndef _IXREF_OBJ_MGR8_H_
#define _IXREF_OBJ_MGR8_H_
...
public:
	...
	*/
	...
	\li Have the same source file (see IObjXRefRecord::GetSrcFileName), 
	...
	*/
	virtual bool SetRecordSrcFile(IObjXRefRecord& xrefRecord, const TCHAR* srcFileName) = 0;
	...
	*/
	virtual bool SetXRefItemSrcName(ReferenceTarget& xrefItem, const TCHAR* srcItemName) = 0;
	...
	\see IObjXRefRecord::Update, IXRefItem::GetSrcItem
	*/
	virtual bool SetProxyItemSrcName(ReferenceTarget& xrefProxy, const TCHAR* proxyName) = 0;

	//! Sets a new file path and name for the specified xref item
	...
	\see IObjXRefRecord::Update, IXRefItem::GetSrcItem
	*/
	virtual bool SetXRefItemSrcFile(ReferenceTarget& xrefItem, const TCHAR* srcFileName) = 0;
	//! Sets a new file path and name for the specified xref proxy
	/*! Call this method in order to change the file a proxy scene entity is xrefed from.
	Xref records could be removed if they become empty as a result of executing this method.
	\param [in] xrefProxy - The xref proxy to be changed. 
	\param [in] proxyFileName - The path and name of the new proxy file. Must be a non-empty string.
	This method checks that the supplied file path and name is valid for xrefing.
	\return - true if the xref proxy has been successfully updated, false otherwise. 
	If the source item has not been successfully updated, IXRefItem::GetSrcItem will return NULL.
	\see IObjXRefRecord::Update, IXRefItem::GetSrcItem
	*/
	virtual bool SetProxyItemSrcFile(ReferenceTarget& xrefProxy, const TCHAR* proxyFileName) = 0;
	//@}
	...
	*/
	...
	BEGIN_FUNCTION_MAP
		...
		FN_2(kfpSetXRefItemSrcFile,	TYPE_bool,	SetXRefItemSrcFile,	TYPE_REFTARG_BR, TYPE_FILENAME);
		FN_2(kfpSetProxyItemSrcFile,	TYPE_bool,	SetProxyItemSrcFile,	TYPE_REFTARG_BR, TYPE_FILENAME);
	...
	private:
		bool FPSetRecordSrcFile(FPInterface* xrefRecord, const TCHAR* srcFileName);
