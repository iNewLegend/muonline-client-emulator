...
void lua_tinker::print_error(lua_State *L, const char* fmt, ...)
{
	...
	{
		MessageBoxA(NULL,text, "lua error", 0);
		printf("%s\n", text);
		lua_pop(L, 1);
	}
}
...
{
	return (wchar_t*)lua_tostring(L, index);				
}
...
{
	return (const wchar_t*)lua_tostring(L, index);		
}

template<>
void* lua_tinker::read(lua_State *L, int index)
{
	return (void*)lua_touserdata(L, index);	///	error		
}
...
{
	lua_pushlstring(L, (char*)ret,wcslen(ret)*2+2);// 因为宽字节补尾需要2字节 所以lua默认只补了1个字节 造成错误（修正：长度+2）
}
...
{
	lua_pushlstring(L, (const char*)ret,wcslen(ret)*2+2);
}

template<>
void lua_tinker::push(lua_State *L, void* ret)
{
