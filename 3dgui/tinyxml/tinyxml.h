...
public:
	...
	*/
	...
	/** Given an attribute name, Attribute() returns the value
		for the attribute of that name, or null if none exists.
		If the attribute exists and can be converted to a bool,
		the bool value will be put in the return 'b', if 'b'
		is non-null.
	*/
	const char* Attribute( const char* name, bool* b ) const;
...
public:
	...
	*/
	...
	bool LoadFormMemory( char* buf, size_t length, TiXmlEncoding encoding = TIXML_DEFAULT_ENCODING );
